  ...
  if (mine.length === 0) {
    return (
      <div className="border border-dashed border-canvas-border rounded-[12px] p-10 text-center">
        ...
        <p className="text-[13px] font-semibold text-ink-700">No work assigned to {userName(currentUserId)}</p>
        <p className="text-[12px] text-ink-500 mt-1">Switch "Acting as" to a user who has assignments, or assign exceptions from the Exceptions tab.</p>
      </div>
    );
  }
    ...
    <div className="border border-canvas-border rounded-[12px] bg-canvas-elevated overflow-hidden">
      <div className="px-5 py-3.5 border-b border-canvas-border flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[12.5px] text-ink-700 truncate max-w-[420px]">{ex.title}</span>
        </div>
        <div className="flex items-center gap-2">
          <span className="text-[11px] text-ink-500">via {assignment.workflowName}</span>
          {assignment.dueDate && <span className="inline-flex items-center gap-1 h-6 px-2 text-[11px] text-ink-700 bg-[#FAFAFB] border border-canvas-border rounded-full"><Calendar size={10} /> {assignment.dueDate}</span>}
        </div>
      </div>
      ...
      <div className="grid md:grid-cols-[1fr_240px]">
        <div className="p-5 space-y-4">
          {assignment.note && (
            <div className="flex items-start gap-2 px-3 py-2.5 bg-brand-50/40 border border-brand-100 rounded-[8px]">
              <MessageSquare size={13} className="text-brand-700 mt-0.5 shrink-0" />
              <div className="text-[12px] text-ink-700"><span className="font-semibold text-brand-700">{userName(assignment.assignedBy)}:</span> {assignment.note}</div>
            </div>
          )}
          {rejected && (
            <div className="flex items-start gap-2 px-3 py-2.5 bg-risk-50 border border-risk/30 rounded-[8px]">
              <RotateCcw size={13} className="text-risk-700 mt-0.5 shrink-0" />
              <div className="text-[12px] text-risk-700">Returned for revision. Update and resubmit.</div>
            </div>
          )}
          ...
          <div className="grid grid-cols-2 gap-x-5 gap-y-3">
            {visible.filter(p => !(isEditable(p.key) && (p.key === 'classification' || p.key === 'dueDate'))).map(p => (
              <div key={p.key}>
                <div className="text-[10.5px] font-semibold uppercase tracking-wide text-ink-400 mb-0.5 flex items-center gap-1"><Lock size={9} /> {p.label}</div>
          ...
          {(needsClassification || needsDue) && (
            <div className="grid grid-cols-2 gap-4 pt-1">
              {needsClassification && (
                <div>
                  <label className="text-[12px] font-semibold text-ink-800 mb-1.5 block">Classification <span className="text-risk">*</span></label>
                  <select value={classification} onChange={e => setField({ classification: e.target.value })} className="w-full h-9 px-2.5 bg-canvas-elevated border border-canvas-border rounded-[8px] text-[12.5px] text-ink-900 focus:outline-none focus:border-brand-600 cursor-pointer">
              ...
              {needsDue && (
                <div>
                  <label className="text-[12px] font-semibold text-ink-800 mb-1.5 block">Action Due Date <span className="text-risk">*</span></label>
          ...
          {showActionPlan && (
            <div className="space-y-3 border-t border-dashed border-canvas-border pt-3">
              <div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-ink-500">Action Plan</div>
              <input value={draft.actionName ?? ''} onChange={e => setField({ actionName: e.target.value })} placeholder="Action name *" className="w-full h-9 px-2.5 bg-canvas-elevated border border-canvas-border rounded-[8px] text-[12.5px] focus:outline-none focus:border-brand-600" />
              <textarea value={draft.actionDetails ?? ''} onChange={e => setField({ actionDetails: e.target.value })} rows={2} placeholder="Action details — remediation steps, evidence *" className="w-full resize-none p-2.5 bg-canvas-elevated border border-canvas-border rounded-[8px] text-[12.5px] focus:outline-none focus:border-brand-600" />
            </div>
          )}

          <div className="flex items-center justify-between pt-1">
            <span className="text-[11px] text-ink-400">Next: {assignment.levels[0] ? `${assignment.levels[0].name} (${userById(assignment.levels[0].assigneeIds[0])?.name ?? '—'})` : '—'}</span>
            <button
              onClick={() => { submitForApproval(assignment.id, draft); addToast({ type: 'success', message: `${ex.id} submitted for approval.` }); }}
              disabled={!canSubmit}
              className="h-9 px-4 inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-white bg-brand-600 hover:bg-brand-500 rounded-[8px] cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
        ...
        <div className="p-5 bg-[#FAFAFB] border-t md:border-t-0 md:border-l border-canvas-border">
          <div className="text-[11px] font-semibold uppercase tracking-[0.1em] text-ink-500 mb-3">Approval chain</div>
