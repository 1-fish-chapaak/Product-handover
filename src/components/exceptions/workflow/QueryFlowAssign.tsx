  ...
  return (
    <>
      <button
        ...
        className={`group inline-flex items-center gap-1.5 h-8 pl-2.5 pr-2.5 rounded-md text-[0.75rem] leading-4 font-semibold cursor-pointer transition-colors ${
          count > 0
      ...
      >
        ...
        {count > 0 && <span className="inline-flex items-center justify-center h-4 min-w-4 px-1 rounded-full bg-evidence-600 text-white text-[0.625rem] font-semibold tabular-nums">{count}</span>}
      </button>
...
function QueryFlowModal({ queryId, onClose }: { queryId: string; onClose: () => void }) {
  ...
  const SideTab = ({ p }: { p: Persona }) => {
    ...
    return (
      <button
        onClick={() => setSide(p)}
        className={`h-8 px-3 inline-flex items-center gap-1.5 text-[0.75rem] font-semibold rounded-sm cursor-pointer transition-colors ${side === p ? 'bg-canvas-elevated text-brand-700 shadow-sm' : 'text-ink-500 hover:text-ink-800'}`}
      >
      ...
      >
        <header className="shrink-0 px-6 py-3 flex items-center justify-between gap-4 border-b border-canvas-border">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg bg-brand-50 text-brand-700 flex items-center justify-center"><GitBranch size={16} /></div>
            <div>
          ...
          <div className="p-6 space-y-5 border-b md:border-b-0 md:border-r border-canvas-border">
            <div>
              <div className="flex items-center justify-between gap-2 mb-1.5">
                <label className="text-[0.75rem] font-semibold text-ink-800">{side === 'auditor' ? 'Auditor' : 'Risk Owner'} approval flow</label>
                {assignedRecord && (
                  <span className="inline-flex items-center gap-1 h-5 px-2 text-[0.625rem] font-semibold bg-compliant-50 text-compliant-700 rounded-full">
              ...
              <select
                ...
                className="w-full h-10 px-3 bg-canvas-elevated border border-canvas-border rounded-md text-[0.8125rem] text-ink-900 focus:outline-none focus:border-brand-600 cursor-pointer"
              >
              ...
              {sideFlows.length === 0 && (
                <p className="mt-1.5 text-[0.6875rem] text-mitigated-700">No {side === 'auditor' ? 'Auditor' : 'Risk Owner'} flows exist yet — create one to assign it here.</p>
              )}
              {assignedRecord && (
              ...
              <button
                ...
                className="mt-3 inline-flex items-center gap-1.5 text-[0.75rem] font-semibold text-brand-700 hover:text-brand-800 cursor-pointer"
              >
          ...
          <div className="p-6 space-y-5">
            <div>
              ...
              {template ? (
                ...
                <p className="text-[0.75rem] text-ink-500">Select an approval flow on the left to preview its approval chain.</p>
              )}
            </div>

            <div>
              <div className="text-[0.75rem] font-semibold text-ink-800 mb-2">Query</div>
              <span className="inline-flex items-center h-6 px-2.5 text-[0.71875rem] font-mono font-medium bg-brand-50 text-brand-700 rounded-full">{queryId}</span>
            </div>
          </div>
        </div>
        ...
        <footer className="shrink-0 px-6 py-3.5 border-t border-canvas-border flex items-center justify-between gap-2">
          {assignedRecord ? (
            <button onClick={remove} className="inline-flex items-center gap-1.5 text-[0.71875rem] font-medium text-risk-700 hover:text-risk-800 cursor-pointer">
              <Trash2 size={13} /> Remove {side === 'auditor' ? 'Auditor' : 'Risk Owner'} flow
            </button>
          ) : (
            <span className="text-[0.71875rem] text-ink-500">Pick a flow, then assign it to {queryId}.</span>
          )}
          <div className="flex items-center gap-2">
            <button onClick={onClose} className="h-10 px-5 text-[0.78125rem] font-medium text-ink-700 bg-canvas-elevated border border-canvas-border rounded-md hover:border-brand-200 cursor-pointer">Close</button>
            <button
              onClick={assign}
              disabled={!template || alreadyAssignedSame}
              className="h-10 px-5 inline-flex items-center gap-2 text-[0.78125rem] font-semibold text-white bg-brand-600 hover:bg-brand-500 rounded-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
            >
