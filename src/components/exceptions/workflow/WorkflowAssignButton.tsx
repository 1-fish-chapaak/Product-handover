...
export default function WorkflowAssignButton({ selectedIds }: { selectedIds: string[] }) {
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 h-8 px-2.5 text-[0.75rem] font-medium rounded-md border transition-colors ${
        active
    ...
    >
      ...
      {active && (
        <span className="inline-flex items-center h-5 min-w-5 px-1 text-[0.65625rem] font-semibold bg-brand-50 rounded-full tabular-nums">{selectedIds.length}</span>
      )}
    </button>
  );
}
