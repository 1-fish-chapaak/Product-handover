      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto grid md:grid-cols-2">
          ...
          <div className="p-6 space-y-5 border-b md:border-b-0 md:border-r border-canvas-border">
            <div>
              <label className="text-[12px] font-semibold text-ink-800 mb-1.5 block">Approval Flow <span className="text-risk">*</span></label>
              <select value={effectiveWorkflowId} onChange={e => setWorkflowId(e.target.value)} className="w-full h-10 px-3 bg-canvas-elevated border border-canvas-border rounded-[8px] text-[13px] text-ink-900 focus:outline-none focus:border-brand-600 cursor-pointer">
              ...
              {sideTemplates.length === 0 && (
                <p className="mt-1.5 text-[11px] text-mitigated-700">No {role === 'auditor' ? 'Auditor' : 'Risk Owner'} flows exist yet — create one to assign it here.</p>
              )}
              {template && (
              ...
              <button onClick={createNewFlow} className="mt-3 inline-flex items-center gap-1.5 text-[12px] font-semibold text-brand-700 hover:text-brand-800 cursor-pointer">
                <Plus size={14} /> Create new approval flow
              </button>
            </div>

            {!isAuditorSide && (
              <p className="text-[11.5px] text-ink-500 bg-[#FAFAFB] border border-canvas-border rounded-[8px] px-3 py-2">
          ...
          <div className="p-6 space-y-5">
            <div>
              ...
              {template ? (
                ...
                <p className="text-[12px] text-ink-500">Pick an approval flow to preview its chain.</p>
              )}
            </div>

            <div>
              <div className="text-[12px] font-semibold text-ink-800 mb-2">Exception{ids.length === 1 ? '' : 's'} being sent <span className="text-ink-400 font-normal tabular-nums">({ids.length})</span></div>
              <div className="flex flex-wrap gap-1.5">
                {ids.map(id => (
                  <span key={id} className="inline-flex items-center h-6 px-2.5 text-[11.5px] font-mono font-medium bg-brand-50 text-brand-700 rounded-full">{id}</span>
                ))}
              </div>
            </div>
          </div>
        ...
        <footer className="shrink-0 px-6 py-3.5 border-t border-canvas-border flex items-center justify-between gap-2">
          {alreadyAssigned ? (
            <button onClick={removeFlow} className="inline-flex items-center gap-1.5 text-[11.5px] font-medium text-risk-700 hover:text-risk-800 cursor-pointer">
              <Trash2 size={13} /> Remove {role === 'auditor' ? 'Auditor' : 'Risk Owner'} flow
            </button>
          ) : (
            <span className="text-[11.5px] text-ink-500">{ids.length} exception{ids.length === 1 ? '' : 's'} → {template?.name ?? 'select an approval flow'}</span>
          )}
