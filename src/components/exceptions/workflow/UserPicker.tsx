...
function Avatar({ u }: { u: OrgUser }) {
  return (
    <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-[0.625rem] font-semibold shrink-0 ${u.active ? 'bg-brand-50 text-brand-700' : 'bg-risk-50 text-risk-700'}`}>
      {u.initials}
    </span>
  );
}
...
export function UserSelect({ users, value, onChange, placeholder = 'Select a user…' }: {
  ...
  return (
    <div ref={ref} className="relative">
      <button type="button" onClick={() => setOpen(o => !o)} className="w-full h-10 px-3 bg-canvas-elevated border border-canvas-border rounded-md text-[0.8125rem] flex items-center justify-between gap-2 hover:border-brand-200 cursor-pointer">
        {sel ? (
          <span className="flex items-center gap-2 min-w-0"><Avatar u={sel} /><span className="truncate text-ink-800">{sel.name}</span><span className="text-ink-400 text-[0.6875rem] truncate">· {sel.role}</span>{!sel.active && <span className="text-[0.625rem] text-risk-700 inline-flex items-center gap-0.5"><UserX size={10}/>inactive</span>}</span>
      ...
      {open && (
        <div className="absolute z-30 top-full mt-1 left-0 right-0 bg-canvas-elevated border border-canvas-border rounded-md shadow-lg overflow-hidden">
          <div className="flex items-center gap-2 px-2.5 py-2 border-b border-canvas-border">
            <Search size={13} className="text-ink-400" />
            <input autoFocus value={q} onChange={e => setQ(e.target.value)} placeholder="Search users…" className="flex-1 text-[0.78125rem] bg-transparent focus:outline-none text-ink-800 placeholder:text-ink-400" />
          </div>
          <div className="max-h-[220px] overflow-y-auto py-1">
            {filtered.map(u => (
              <button key={u.id} type="button" onClick={() => { onChange(u.id); setOpen(false); setQ(''); }} className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-[#FAFAFB] cursor-pointer">
                <Avatar u={u} />
                <span className="flex-1 min-w-0"><span className="text-[0.78125rem] text-ink-800">{u.name}</span> <span className="text-[0.6875rem] text-ink-400">· {u.role}</span></span>
                {!u.active && <span className="text-[0.625rem] text-risk-700 inline-flex items-center gap-0.5"><UserX size={10}/>inactive</span>}
                {u.id === value && <Check size={14} className="text-brand-700" />}
              </button>
            ))}
            {filtered.length === 0 && <div className="px-3 py-3 text-[0.75rem] text-ink-400 text-center">No users found.</div>}
          </div>
        </div>
      )}
    </div>
...
export function UserMultiSelect({ users, selectedIds, onChange }: {
  ...
  return (
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute z-30 top-full mt-1 left-0 right-0 bg-canvas-elevated border border-canvas-border rounded-md shadow-lg overflow-hidden max-h-[240px] overflow-y-auto py-1">
          {users.map(u => {
            ...
            return (
              <button key={u.id} type="button" onClick={() => toggle(u.id)} className="w-full flex items-center gap-2 px-3 py-2 text-left hover:bg-[#FAFAFB] cursor-pointer">
                <span className={`w-4 h-4 rounded-sm border flex items-center justify-center ${on ? 'bg-brand-600 border-brand-600' : 'border-canvas-border'}`}>{on && <Check size={11} className="text-white" />}</span>
                <Avatar u={u} />
                <span className="flex-1 min-w-0"><span className="text-[0.78125rem] text-ink-800">{u.name}</span> <span className="text-[0.6875rem] text-ink-400">· {u.role}</span></span>
                {!u.active && <span className="text-[0.625rem] text-risk-700 inline-flex items-center gap-0.5"><UserX size={10}/>inactive</span>}
              </button>
