...
function ContinuousMonitoringDashboard({ engagement, state, onNavigateTab }: {
  ...
  return (
    <div className="space-y-5">
      ...
      {/* Schedule warning */}
      {!scheduleActive && (
        <div className="flex items-start gap-2 px-4 py-3 rounded-xl bg-high-50/50 border border-high/30 text-[0.6875rem] text-high-700">
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="space-y-2">
                {exStatusBreakdown.map(s => (
                  <div key={s.name} className="flex items-center gap-2 text-[0.75rem]">
    ...
    <div className="glass-card rounded-2xl p-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-[1rem] font-bold text-ink-900 tracking-tight">Continuous Monitoring Dashboard</h3>
            <p className="text-[0.75rem] text-ink-400">Monitor recurring workflow runs, exceptions, cases, and report readiness.</p>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <span className={`px-3 py-1 rounded-full text-[0.6875rem] font-bold ${statusCls}`}>{statusLabel}</span>
              ...
              {alerts.map((item, i) => {
                ...
                return (
                  <motion.div key={i} initial={{ opacity: 0, x: -6 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
                    ...
                    <span className="text-[0.75rem] text-ink-700">{item.text}</span>
                  </motion.div>
                );
              })}
            </div>
    ...
    <div className="glass-card rounded-2xl overflow-hidden">
      <div className="px-5 py-3.5 border-b border-border-light/50 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-[0.8125rem] font-semibold text-ink-900">Workflow Health</h4>
        </div>
      ...
      <table className="w-full text-[0.75rem]">
        ...
        <tbody>{workflows.map(wf => (
          <tr key={wf.name} className="border-b border-border-light/60 hover:bg-primary/[0.015] transition-colors">
            ...
            <td className="px-4 py-3 text-right"><button onClick={() => onNavigateTab?.('output-review')} className="text-[0.6875rem] font-semibold text-primary hover:underline cursor-pointer">Review</button></td>
          </tr>
        ))}</tbody>
      </table>
    </div>
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <button key={a.tabId} onClick={() => onNavigateTab?.(a.tabId)}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-canvas-border text-[0.6875rem] font-medium text-ink-500 hover:bg-primary/5 hover:text-primary hover:border-brand-200 cursor-pointer transition-all">
...
function SetupPrompt({ icon: Icon, title, description, cta, onAction }: {
  ...
  return (
    <div className="glass-card rounded-2xl p-8 text-center space-y-3">
      ...
      <button onClick={onAction} className="px-5 py-2.5 rounded-xl bg-gradient-to-r from-primary to-primary-medium text-white text-[0.8125rem] font-semibold hover:from-primary-hover hover:to-primary transition-all cursor-pointer inline-flex items-center gap-1.5">
    ...
    if (hasDashboardOutput && !hasRunData && !hasScheduledRuns) {
      return (
        <div className="space-y-4">
          ...
          <div className="flex items-center justify-between px-4 py-3 rounded-lg bg-blue-50 border border-blue-200 text-[0.6875rem] text-blue-700">
