...
export default function CommonDetailsStep({ patternType, details, onChange, reviewerRequired }: Props) {
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-[0.9375rem] font-bold text-text mb-1">{label} Details</h3>
        <p className="text-[0.75rem] text-text-muted">Provide the basic information for this {label.toLowerCase()}.</p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div>
          ...
          {details.dataPeriodStart && details.dataPeriodEnd && details.dataPeriodStart > details.dataPeriodEnd && (
            <p className="text-[0.625rem] text-red-500 mt-1">Data Period From should not be after Data Period To.</p>
          )}
        </div>
      </div>
    </div>
