    ...
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          <span className="px-2 py-0.5 rounded bg-primary/10 text-primary text-[0.5625rem] font-bold">{cfg.framework.replace(/_/g, ' ')}</span>
          <span className="px-2 py-0.5 rounded bg-gray-100 text-gray-600 text-[0.5625rem] font-bold">
                  ...
                  >
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-1.5">
                        <span className="font-mono text-gray-400 text-[0.625rem]">{ctrl.id}</span>
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-1">
                        <span className={`px-1.5 py-0.5 rounded text-[0.5rem] font-bold ${NATURE_CLS[ctrl.nature]}`}>{ctrl.nature}</span>
                        <span className={`px-1.5 py-0.5 rounded text-[0.5rem] font-bold ${AUTO_CLS[ctrl.automation]}`}>{ctrl.automation}</span>
                      </div>
                    </td>
                    ...
                    <td className="px-3 py-2.5 text-center">
                      <span className={`px-2 py-0.5 rounded-full text-[0.5rem] font-bold ${READINESS_CLS[readiness.status]}`}>
                        {readiness.label}
                      </span>
                    </td>
                  </tr>
  ...
  return (
    <div className="rounded-lg border border-border-light bg-surface-2/10 px-4 py-3 flex items-start gap-3">
      ...
      <div className="flex-1 min-w-0">
        <div className="text-[0.75rem] font-semibold text-text mb-0.5">{card.title}</div>
        <div className="text-[0.625rem] text-gray-500">{card.desc}</div>
      </div>
      {card.cta && (
        <button className="px-3 py-1.5 rounded-lg bg-primary/10 text-primary text-[0.625rem] font-semibold hover:bg-primary/20 cursor-pointer transition-colors shrink-0">
          {card.cta}
        </button>
      )}
    </div>
  ...
  return (
    <div className="bg-surface-2/15 border-b border-border-light px-6 py-4 space-y-4">
      ...
      <div className="grid grid-cols-2 gap-4">
        {/* Attributes */}
        <div>
          <h6 className="text-[0.5625rem] font-bold text-gray-400 uppercase tracking-wider mb-1.5">
            Attributes ({ctrl.attributes.length})
          </h6>
          {ctrl.attributes.length === 0 ? (
            <div className="text-[0.625rem] text-red-500 italic">No attributes configured.</div>
          ) : (
            <div className="space-y-1">
              {ctrl.attributes.map(a => {
                const hasWf = !!a.workflowId;
                return (
                  <div key={a.id} className="flex items-center gap-2 text-[0.625rem]">
                    {hasWf ? (
                    ...
                    <span className="text-gray-400 ml-auto text-[0.5625rem]">{a.assertion}</span>
                  </div>
                );
              })}
            </div>
        ...
        <div>
          ...
          {ctrl.workflows.length === 0 ? (
            <div className="text-[0.625rem] text-red-500 italic">No workflows linked.</div>
          ) : (
            <div className="space-y-1.5">
              {ctrl.workflows.map(wf => (
                <div key={wf.id} className="flex items-start gap-2 text-[0.625rem]">
                  <Workflow size={10} className="text-primary mt-0.5 shrink-0" />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5">
                      <span className="text-text font-medium">{wf.name}</span>
                      <span className="text-[0.5rem] font-mono text-gray-400">{wf.version}</span>
                      <span className={`px-1 py-0.5 rounded text-[0.4375rem] font-bold ${AUTO_CLS[wf.type]}`}>{wf.type}</span>
                    </div>
      ...
      {/* Readiness checklist */}
      <div>
        <h6 className="text-[0.5625rem] font-bold text-gray-400 uppercase tracking-wider mb-1.5">Readiness</h6>
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-[0.625rem]">
          {[
        ...
        {readiness.missingItems.length > 0 && (
          <div className="mt-1.5 flex items-start gap-1.5 text-[0.5625rem] text-amber-600">
