  ...
  return (
    <div className="space-y-4">
      ...
      <div>
        ...
        <p className="text-[0.75rem] text-text-muted">Derive and finalize the control conclusion after reviewer approval.</p>
      </div>
      ...
      <div className="grid grid-cols-7 gap-2">
        {[
          ...
          <div key={s.label} className="rounded-lg border border-border-light p-2 text-center">
            <div className={`text-[0.8125rem] font-bold tabular-nums ${s.cls || 'text-text'}`}>{s.value}</div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border-light p-4 space-y-2">
        <h4 className="text-[0.6875rem] font-bold text-text">Conclusion Rationale</h4>
        <p className="text-[0.6875rem] text-text leading-relaxed">{derivation.reason}</p>
        <div className="grid grid-cols-3 gap-3 text-[0.6875rem]">
          <div><span className="text-gray-400 block text-[0.625rem]">Samples Tested</span><span className="text-text font-medium tabular-nums">{derivation.totalSamples}</span></div>
          <div><span className="text-gray-400 block text-[0.625rem]">Passed</span><span className="text-emerald-600 font-medium tabular-nums">{derivation.passedSamples}</span></div>
          <div><span className="text-gray-400 block text-[0.625rem]">Failed</span><span className={`font-medium tabular-nums ${derivation.failedSamples > 0 ? 'text-red-600' : 'text-gray-400'}`}>{derivation.failedSamples}</span></div>
        </div>
        {derivation.recommendedConclusion && (
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[0.625rem] text-gray-400">System Recommendation:</span>
            <span className={`px-2 py-0.5 rounded-full text-[0.625rem] font-bold border ${CONCLUSION_DISPLAY[derivation.recommendedConclusion].cls}`}>
      ...
      <div className="rounded-lg border-2 border-primary/20 bg-primary/5 p-4 space-y-3">
        <h4 className="text-[0.75rem] font-bold text-text">Finalize Conclusion</h4>
        <div>
          <label className="text-[0.625rem] font-semibold text-gray-500 block mb-1.5">Final Conclusion</label>
            ...
            {(['EFFECTIVE', 'PARTIALLY_EFFECTIVE', 'INEFFECTIVE', 'NOT_APPLICABLE'] as ConclusionValue[]).map(v => {
              ...
              return (
                <button key={v} onClick={() => setSelectedValue(v)}
                  className={`px-3 py-1.5 rounded-lg text-[0.6875rem] font-semibold cursor-pointer transition-all border-2 ${isSelected ? d.cls + ' ring-2 ring-primary/20' : 'border-border-light text-gray-400 hover:border-gray-300'}`}>
                  {d.label}
                </button>
              );
            })}
          ...
          {derivation.recommendedConclusion && selectedValue === derivation.recommendedConclusion && (
            <p className="text-[0.5625rem] text-gray-400 mt-1">Matches system recommendation.</p>
          )}
          {differsFromRecommendation && (
            <p className="text-[0.5625rem] text-amber-600 mt-1 flex items-center gap-1"><AlertTriangle size={9} />Differs from system recommendation. Remarks are required.</p>
          )}
        </div>
        <div>
          <label className="text-[0.625rem] font-semibold text-gray-500 block mb-1">Remarks / Justification {needsRemarks && <span className="text-red-400">*</span>}</label>
  ...
  return (
    <div className="space-y-4">
        ...
        <div className={`rounded-xl border-2 p-5 ${d.cls}`}>
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-[0.6875rem] mt-0.5">Finalized by {ctrlConclusion.finalizedBy} on {ctrlConclusion.finalizedAt}</p>
            </div>
          </div>
          {ctrlConclusion.remarks && <p className="text-[0.6875rem] italic mt-2">"{ctrlConclusion.remarks}"</p>}
        </div>
      )}
      ...
      <div className="rounded-lg border border-border-light p-4">
        <h4 className="text-[0.6875rem] font-bold text-text mb-1">Conclusion Rationale</h4>
        <p className="text-[0.6875rem] text-text leading-relaxed">{ctrlConclusion.reason}</p>
        <div className="grid grid-cols-3 gap-3 text-[0.6875rem] mt-2">
          <div><span className="text-gray-400 block text-[0.625rem]">Samples</span><span className="text-text font-medium tabular-nums">{derivation.totalSamples}</span></div>
          <div><span className="text-gray-400 block text-[0.625rem]">Passed</span><span className="text-emerald-600 font-medium tabular-nums">{derivation.passedSamples}</span></div>
          <div><span className="text-gray-400 block text-[0.625rem]">Failed</span><span className={`font-medium tabular-nums ${derivation.failedSamples > 0 ? 'text-red-600' : 'text-gray-400'}`}>{derivation.failedSamples}</span></div>
        </div>
      </div>
      ...
      {ctrlConclusion.history.length > 0 && (
        <div className="rounded-lg border border-border-light p-4">
          <h4 className="text-[0.6875rem] font-bold text-text mb-2">Conclusion History</h4>
            ...
            {ctrlConclusion.history.map(h => (
              <div key={h.id} className="flex items-start gap-2 text-[0.625rem]">
                ...
                <div>
                  ...
                  <span className="ml-2 px-1.5 py-0.5 rounded text-[0.5rem] font-bold border">{CONCLUSION_DISPLAY[h.value].label}</span>
      ...
      {!showUpdate ? (
        <button onClick={() => setShowUpdate(true)}
          className="text-[0.625rem] font-medium text-gray-400 hover:text-primary cursor-pointer transition-colors">
          Update Conclusion...
        </button>
      ) : (
        <div className="rounded-lg border border-border-light p-4 space-y-3">
          <h4 className="text-[0.6875rem] font-bold text-text">Update Conclusion</h4>
          <div className="flex items-center gap-2">
            {(['EFFECTIVE', 'PARTIALLY_EFFECTIVE', 'INEFFECTIVE', 'NOT_APPLICABLE'] as ConclusionValue[]).map(v => (
              <button key={v} onClick={() => setUpdateValue(v)}
                className={`px-2.5 py-1 rounded-lg text-[0.625rem] font-semibold cursor-pointer border-2 transition-all ${updateValue === v ? CONCLUSION_DISPLAY[v].cls + ' ring-2 ring-primary/20' : 'border-border-light text-gray-400'}`}>
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => {
              ...
              className="px-3 py-1.5 rounded-lg bg-primary hover:bg-primary/90 text-white text-[0.625rem] font-semibold cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
              Update
            </button>
            <button onClick={() => setShowUpdate(false)} className="px-3 py-1.5 rounded-lg border border-border-light text-[0.625rem] font-medium text-text-muted hover:bg-surface-2/30 cursor-pointer transition-colors">Cancel</button>
          </div>
        </div>
      )}
    </div>
...
function FailedAttributesTable({ failedAttributes }: { failedAttributes: ConclusionDerivation['failedAttributes'] }) {
  if (failedAttributes.length === 0) {
    return (
      <div className="rounded-lg border border-border-light p-3 text-[0.6875rem] text-gray-400 italic flex items-center gap-2">
  ...
  return (
    <div className="rounded-lg border border-border-light overflow-hidden">
      <div className="px-3 py-2 bg-red-50/30 border-b border-border-light">
        <h4 className="text-[0.625rem] font-bold text-red-700">Failed Samples / Attributes ({failedAttributes.length})</h4>
      </div>
        ...
        <tbody>
          {failedAttributes.map((fa, i) => (
            <tr key={i} className="border-b border-border-light/50">
              ...
              <td className="px-2 py-1.5"><span className="px-1 py-0.5 rounded bg-primary/10 text-primary text-[0.4375rem] font-bold">{fa.assertion}</span></td>
