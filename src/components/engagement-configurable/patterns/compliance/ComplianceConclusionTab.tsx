  ...
  if (testItems.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <Lock size={24} className="text-ink-300 mb-3" />
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          ...
          { label: 'Review', value: ctrlReview.status.replace(/_/g, ' '), cls: isApproved ? 'text-emerald-600' : 'text-ink-400' },
        ].map(s => (
          ...
          <div className="flex items-center gap-1.5">
            {LIKELIHOOD_OPTIONS.map(l => (
              <button key={l} onClick={() => setLikelihood(l)}
                className={`px-2.5 py-1.5 rounded-lg text-[0.6875rem] font-semibold cursor-pointer transition-colors border ${likelihood === l ? 'bg-primary text-white border-primary' : 'bg-white text-ink-500 border-border-light hover:border-primary/30'}`}>
                {l}
              </button>
            ))}
          </div>
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-lg border border-border-light p-4 space-y-2">
        ...
        <div className="grid grid-cols-3 gap-3 text-[0.75rem]">
          ...
          <div><span className="text-text-muted block text-[0.6875rem]">Failed</span><span className={`font-medium tabular-nums ${derivation.failedSamples > 0 ? 'text-red-600' : 'text-ink-400'}`}>{derivation.failedSamples}</span></div>
        </div>
      ...
      <div className="rounded-lg border-2 border-primary/20 bg-primary/5 p-4 space-y-3">
        ...
        <div>
          <label className="text-[0.75rem] font-semibold text-ink-500 block mb-1.5">Final Conclusion</label>
            ...
            {(['EFFECTIVE', 'PARTIALLY_EFFECTIVE', 'INEFFECTIVE', 'NOT_APPLICABLE'] as ConclusionValue[]).map(v => {
              ...
              return (
                <button key={v} onClick={() => setSelectedValue(v)}
                  className={`px-3 py-1.5 rounded-lg text-[0.75rem] font-semibold cursor-pointer transition-all border-2 ${isSelected ? d.cls + ' ring-2 ring-primary/20' : 'border-border-light text-ink-400 hover:border-ink-300'}`}>
                  {d.label}
                </button>
              );
            })}
        ...
        <div>
          <label className="text-[0.75rem] font-semibold text-ink-500 block mb-1">Remarks / Justification {needsRemarks && <span className="text-red-400">*</span>}</label>
    ...
    <div className="space-y-4">
      ...
      {!showUpdate ? (
        <button onClick={() => setShowUpdate(true)}
          className="text-[0.75rem] font-medium text-ink-400 hover:text-primary cursor-pointer transition-colors">
          Update Conclusion...
        </button>
      ) : (
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {(['EFFECTIVE', 'PARTIALLY_EFFECTIVE', 'INEFFECTIVE', 'NOT_APPLICABLE'] as ConclusionValue[]).map(v => (
              <button key={v} onClick={() => setUpdateValue(v)}
                className={`px-2.5 py-1 rounded-lg text-[0.6875rem] font-semibold cursor-pointer border-2 transition-all ${updateValue === v ? CONCLUSION_DISPLAY[v].cls + ' ring-2 ring-primary/20' : 'border-border-light text-ink-400'}`}>
...
function FailedAttributesTable({ failedAttributes }: { failedAttributes: ConclusionDerivation['failedAttributes'] }) {
  if (failedAttributes.length === 0) {
    return (
      <div className="rounded-lg border border-border-light p-3 text-[0.75rem] text-ink-400 italic flex items-center gap-2">
        ...
        <tbody>
          {failedAttributes.map((fa, i) => (
            <tr key={i} className="border-b border-border-light/50">
              <td className="px-2 py-1.5 font-mono text-ink-500">{fa.testItemRef}</td>
              ...
              <td className="px-2 py-1.5 text-ink-500 truncate max-w-[240px]" title={fa.notes || undefined}>{fa.notes || '—'}</td>
            </tr>
          ))}
        </tbody>
      </table>
