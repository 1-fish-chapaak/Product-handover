  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        <span className="px-2.5 py-1 rounded-lg bg-gray-50 border border-border-light text-[0.625rem] text-gray-500 font-medium">Review: <span className="font-bold">Not Submitted</span></span>
        <span className="px-2.5 py-1 rounded-lg bg-gray-50 border border-border-light text-[0.625rem] text-gray-500 font-medium">Conclusion: <span className="font-bold">Locked</span></span>
      </div>
        ...
        <div className="rounded-xl border border-border-light bg-white p-5 space-y-4">
          ...
          <Section num={1} title="Working Paper Header">
            <div className="grid grid-cols-3 gap-3 text-[0.6875rem]">
              <div><span className="text-gray-400 block text-[0.625rem]">Engagement</span><span className="text-text font-medium">{engagement.name}</span></div>
              <div><span className="text-gray-400 block text-[0.625rem]">Framework</span><span className="text-text font-medium">{cfg.framework.replace(/_/g, ' ')}</span></div>
              <div><span className="text-gray-400 block text-[0.625rem]">Audit Type</span><span className="text-text font-medium">{cfg.auditType}</span></div>
          ...
          <Section num={2} title="Control Objective">
            <div className="text-[0.6875rem] space-y-1">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className={`px-1.5 py-0.5 rounded text-[0.5rem] font-bold ${selectedControl.nature === 'Preventive' ? 'bg-emerald-50 text-emerald-700' : 'bg-blue-50 text-blue-700'}`}>{selectedControl.nature}</span>
                <span className={`px-1.5 py-0.5 rounded text-[0.5rem] font-bold ${selectedControl.automation === 'Automated' ? 'bg-purple-50 text-purple-700' : selectedControl.automation === 'Hybrid' ? 'bg-indigo-50 text-indigo-700' : 'bg-gray-100 text-gray-600'}`}>{selectedControl.automation}</span>
              </div>
          ...
          <Section num={3} title="Test Design">
            <div className="grid grid-cols-3 gap-3 text-[0.6875rem]">
              <div><span className="text-gray-400 block text-[0.625rem]">Input Method</span><span className="text-text font-medium">{cfg.defaultTestingInputMethod.replace(/_/g, ' ').toLowerCase().replace(/\b\w/g, c => c.toUpperCase())}</span></div>
              ...
              <div><span className="text-gray-400 block text-[0.625rem]">Testing Status</span><span className={`font-medium ${summary.completionPercent === 100 ? 'text-emerald-600' : summary.completedChecks > 0 ? 'text-amber-600' : 'text-gray-500'}`}>{summary.completionPercent === 100 ? 'Complete' : summary.completedChecks > 0 ? 'In Progress' : 'Not Started'} ({summary.completionPercent}%)</span></div>
            </div>
          </Section>
                  ...
                  {attrLegend.map(a => {
                    ...
                    return (
                      <tr key={a.id} className="border-b border-border-light/50">
                        ...
                        <td className="px-2 py-1.5"><span className="px-1.5 py-0.5 rounded bg-primary/10 text-primary text-[0.5rem] font-bold">{a.assertion}</span></td>
                        <td className="px-2 py-1.5 text-text">{a.name}</td>
                        <td className="px-2 py-1.5 text-gray-500">{wf?.name || '—'} {wf?.version || ''}</td>
                        <td className="px-2 py-1.5"><span className={`px-1 py-0.5 rounded text-[0.4375rem] font-bold ${wf?.type === 'Automated' ? 'bg-purple-50 text-purple-700' : 'bg-gray-100 text-gray-600'}`}>{wf?.type || 'Manual'}</span></td>
                        <td className="px-2 py-1.5 text-gray-500 text-[0.5625rem]">{wf?.type === 'Automated' || wf?.type === 'Hybrid' ? 'System + User' : 'User Evidence'}</td>
                      </tr>
                    );
                  })}
                </tbody>
          ...
          <Section num={5} title="Sample Data Summary">
            <div className="grid grid-cols-3 gap-3 text-[0.6875rem]">
              <div><span className="text-gray-400 block text-[0.625rem]">Test Items</span><span className="text-text font-medium tabular-nums">{ctrlTestItems.length}</span></div>
              <div><span className="text-gray-400 block text-[0.625rem]">Source</span><span className="text-text font-medium">{complianceState.samplesEvidence.batches[0]?.inputMethod || 'Uploaded'}</span></div>
              <div><span className="text-gray-400 block text-[0.625rem]">Batch</span><span className="text-text font-medium">{complianceState.samplesEvidence.batches[0]?.name || '—'}</span></div>
            </div>
          </Section>

          {/* 6. Evidence Coverage Matrix */}
          <Section num={6} title="Evidence Coverage Matrix">
            <p className="text-[0.5625rem] text-gray-400 mb-2">Attribute columns show user/PBC-uploaded files. System Evidence shows workflow logs.</p>
            <div className="rounded-lg border border-border-light overflow-hidden">
              <table className="w-full text-[0.625rem]">
                <thead><tr className="border-b border-border-light bg-surface-2/30 text-[0.5rem] font-semibold text-gray-400 uppercase">
                  ...
                  {attrLegend.map(a => (
                    <th key={a.id} className="px-2 py-1.5 text-center"><span className="text-primary font-bold text-[0.5625rem]">{a.code}</span></th>
                  ))}
                    ...
                    return (
                      <React.Fragment key={ti.id}>
                        <tr className={`border-b border-border-light/50 cursor-pointer hover:bg-surface-2/20 ${isExp ? 'bg-surface-2/20' : ''}`}
                          ...
                          {attrLegend.map(a => (
                            <td key={a.id} className="px-2 py-1.5 text-center text-[0.5rem]">
                              <span className={`font-medium ${(userByAttr.get(a.code) || 0) > 0 ? 'text-text' : 'text-gray-300'}`}>{userByAttr.get(a.code) || 0}</span>
                            </td>
                          ))}
                          <td className="px-2 py-1.5 text-center text-[0.5rem]"><span className={`font-medium ${sysEv.length > 0 ? 'text-blue-600' : 'text-gray-300'}`}>{sysEv.length}</span></td>
                          <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${TI_EV_CLS[evStatus]}`}>{evStatus}</span></td>
                        </tr>
                        {isExp && tiEvidence.length > 0 && (
                          <tr><td colSpan={attrLegend.length + 4} className="p-0">
                            <div className="bg-surface-2/15 px-4 py-2 border-b border-border-light space-y-1.5">
                              {userEv.length > 0 && (<div>
                                <div className="text-[0.4375rem] font-semibold text-gray-400 uppercase mb-0.5">User / PBC Evidence</div>
                                {userEv.map(e => (
                                  <div key={e.id} className="flex items-center gap-2 text-[0.5625rem]">
                                    ...
                                    <span className={`px-1 py-0.5 rounded text-[0.4375rem] font-bold ${e.source === 'RECEIVED_FROM_PBC' ? 'bg-blue-50 text-blue-600' : 'bg-gray-100 text-gray-600'}`}>{e.source === 'RECEIVED_FROM_PBC' ? 'PBC' : 'User'}</span>
                                  </div>
                                ))}
                              </div>)}
                              {sysEv.length > 0 && (<div>
                                <div className="text-[0.4375rem] font-semibold text-blue-500 uppercase mb-0.5">System Evidence</div>
                                {sysEv.map(e => (
                                  <div key={e.id} className="flex items-center gap-2 text-[0.5625rem]">
                                    <span className="font-bold text-primary">{e.linkedAttributeIds.map(id => attrCodeMap.get(id) || '?').join(',')}</span>
                    ...
                    return (
                      <tr key={ti.id} className="border-b border-border-light/50">
                        ...
                        {attrLegend.map(a => {
                          ...
                          return <td key={a.id} className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.5rem] font-bold ${RESULT_CLS[r]}`}>{RESULT_LABEL[r]}</span></td>;
                        })}
                        <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.5rem] font-bold ${SAMPLE_CLS[sr]}`}>{sr === 'PASS' ? 'Pass' : sr === 'FAIL' ? 'Fail' : 'Pending'}</span></td>
                      </tr>
                    );
                  })}
                </tbody>
                ...
                return (
                  <div key={ti.id} className="rounded-lg border border-border-light overflow-hidden">
                    <button onClick={() => setExpandedDetailSampleId(isExp ? null : ti.id)}
                      ...
                      <span className={`px-1.5 py-0.5 rounded text-[0.5rem] font-bold shrink-0 ${SAMPLE_CLS[sr]}`}>{sr === 'PASS' ? 'Pass' : sr === 'FAIL' ? 'Fail' : 'Pending'}</span>
                      <span className="text-[0.5rem] text-gray-400 shrink-0">{tiEvCount} ev</span>
                      <span className="flex items-center gap-1 ml-auto shrink-0">
                        {attrLegend.map(a => {
                          const ar = ctrlResults.find(r => r.testItemId === ti.id && r.attributeId === a.id);
                          const r = ar?.result || 'NOT_TESTED';
                          return <span key={a.id} className={`px-1 py-0.5 rounded text-[0.4375rem] font-bold ${RESULT_CLS[r]}`}>{a.code}:{RESULT_LABEL[r]}</span>;
                        })}
                      </span>
                    </button>
                    {isExp && (
                            ...
                            {attrLegend.map(a => {
                              ...
                              return (
                                <tr key={a.id} className="border-t border-border-light/30">
                                  ...
                                  <td className="py-1 pr-2"><span className="px-1 py-0.5 rounded bg-primary/10 text-primary text-[0.4375rem] font-bold">{a.assertion}</span></td>
                                  <td className="py-1 pr-2 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.5rem] font-bold ${RESULT_CLS[r]}`}>{RESULT_LABEL[r]}</span></td>
                    ...
                    <tbody>
                      {ctrlRequests.map(r => (
                        <tr key={r.id} className="border-b border-border-light/50">
                          ...
                          <td className="px-2 py-1.5"><span className="px-1.5 py-0.5 rounded bg-primary/10 text-primary text-[0.5rem] font-bold">{r.requestType}</span></td>
                          <td className="px-2 py-1.5 text-gray-500">{r.requestedFrom}</td>
                          <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${r.status === 'Received' ? 'bg-emerald-50 text-emerald-700' : r.status === 'Overdue' ? 'bg-red-50 text-red-700' : 'bg-gray-100 text-gray-500'}`}>{r.status}</span></td>
