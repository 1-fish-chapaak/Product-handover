...
export default function AutomationWorkflowsTab({ engagement, inputData, setup, runsState, onUpdateSetup, onUpdateRuns, onNavigateTab }: Props) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className="pb-4">
        <div className="font-mono text-[0.6875rem] text-text-muted mb-1 tracking-tight">Workflow Library</div>
        <h2 className="text-[1.375rem] font-semibold text-text tracking-tight leading-tight">Workflows</h2>
        <p className="text-[0.8125rem] text-text-muted mt-1">Browse the workflow catalog and add the ones relevant to your project.</p>
      </div>

      {/* Search + Actions */}
      <div className="pb-4 flex items-center gap-3">
        {wfIds.length > 0 && <span className="text-[0.8125rem] text-text-muted"><span className="font-semibold text-text">{wfIds.length}</span> selected</span>}
        ...
        <div className="ml-auto flex items-center gap-3">
          <button className="flex items-center gap-2 px-4 h-10 rounded-md bg-primary-xlight text-primary border border-primary/15 text-[0.8125rem] font-semibold hover:bg-primary/10 transition-colors cursor-pointer">
            <Sparkles size={14} />Create Workflow
          </button>
          <button onClick={() => { if (wfIds.length > 0) setShowBulkModal(true); }} disabled={wfIds.length === 0}
            className="flex items-center gap-2 px-4 h-10 rounded-md bg-primary text-white text-[0.8125rem] font-semibold hover:bg-primary-hover transition-colors cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed">
        ...
        <table className="w-full border-collapse">
          <thead className="bg-white sticky top-0 z-10 border-b border-border-light">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-[0.6875rem] font-semibold uppercase tracking-wider text-text-muted w-[120px]">Business Process</th>
              <th className="px-4 py-3 text-left text-[0.6875rem] font-semibold uppercase tracking-wider text-text-muted w-[140px]">Tags</th>
              <th className="px-4 py-3 text-right text-[0.6875rem] font-semibold uppercase tracking-wider text-text-muted w-[80px]"></th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0 ? (
              <tr><td colSpan={6} className="px-6 py-16 text-center text-[0.8125rem] text-text-muted">No workflows match "{search}"</td></tr>
            ) : filtered.map(wf => {
                  ...
                  <td className="px-4 py-4 align-top w-[320px]">
                    <div className="flex flex-col gap-1">
                      <div className="flex items-start gap-2">
                        <span className="group inline cursor-pointer text-[0.8125rem] text-text font-medium hover:text-primary hover:underline line-clamp-2"
                        ...
                        {wf.live && (
                          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[0.6875rem] font-medium shrink-0 mt-0.5" style={{ backgroundColor: '#ECFEF3', color: '#047A48' }}>
                            <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: '#047A48' }} />Live
                          </span>
                        )}
                        {wf.isCreated && <span className="px-1.5 py-0.5 rounded text-[0.5625rem] font-bold bg-purple-50 text-purple-700 shrink-0 mt-0.5">Project</span>}
                      </div>
                      <span className="text-[0.6875rem] font-mono text-text-muted/70 tracking-tight">{wf.controlId}</span>
                    </div>
                  </td>
                  <td className="px-4 py-4 align-top text-[0.8125rem] text-text-muted max-w-[400px]"><span className="line-clamp-2">{wf.description}</span></td>
                  <td className="px-4 py-4 align-top text-[0.8125rem] text-text-muted">{wf.businessProcess}</td>
                  <td className="px-4 py-4 align-top">
                    <div className="flex flex-wrap gap-1.5">
                      {wf.tags.map(t => <span key={t} className="inline-flex items-center px-2 py-0.5 rounded-md bg-surface-2 border border-border-light text-text-muted text-[0.6875rem] font-medium">{t}</span>)}
                    </div>
                  </td>
                  <td className="px-4 py-4 align-top text-right">
                    <button onClick={e => { e.stopPropagation(); setDetailId(wf.id); }}
                      className="px-2 py-1 rounded text-[0.6875rem] font-medium text-primary hover:bg-primary/10 cursor-pointer transition-colors">View</button>
                  </td>
                </tr>
              );
            })}
      ...
      {summary.total > 0 && (
        <div className="flex items-center justify-between py-3 px-4 border-t border-border-light bg-white">
          ...
          <button onClick={() => onNavigateTab?.('output-review')}
            className="flex items-center gap-1 px-3 py-1.5 rounded-md bg-primary hover:bg-primary-hover text-white text-[0.75rem] font-semibold cursor-pointer transition-colors">
...
const pillCls = (active: boolean) => `px-3.5 h-9 rounded-xl text-[0.8125rem] font-semibold transition-all cursor-pointer ${active ? 'bg-primary text-white shadow-sm' : 'bg-surface-2 text-text-muted hover:bg-surface-2/80 border border-border-light'}`;
...
function WorkflowDetailView({ wf, runs, onBack, onRun }: { wf: ProjectWorkflow; runs: AutomationRun[]; onBack: () => void; onRun: () => void }) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Top */}
      <div className="pb-4">
        <button onClick={onBack} className="flex items-center gap-1.5 text-[0.75rem] text-text-muted hover:text-primary transition-colors cursor-pointer mb-3">
          <ArrowLeft size={14} />Back to Workflows
        </button>
        <div className="flex items-start justify-between gap-4">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-[0.6875rem] font-medium" style={{ backgroundColor: '#ECFEF3', color: '#047A48' }}>
                <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: '#047A48' }} />ACTIVE
              </span>
              <span className="text-[0.6875rem] font-mono text-text-muted">{wf.controlId}</span>
              {wf.isCreated && <span className="px-1.5 py-0.5 rounded text-[0.5625rem] font-bold bg-purple-50 text-purple-700">Created in Project</span>}
            </div>
            <h2 className="text-[1.375rem] font-semibold text-text tracking-tight leading-tight mb-1">{wf.name}</h2>
            {lastRun && <div className="flex items-center gap-1.5 text-[0.75rem] text-text-muted"><Clock size={12} />Last run: {lastRun.completedAt || 'Pending'}</div>}
          </div>
          <button onClick={onRun}
            className="flex items-center gap-2 px-4 h-10 rounded-md bg-primary text-white text-[0.8125rem] font-semibold hover:bg-primary-hover transition-colors cursor-pointer shrink-0">
      ...
      <div className="flex items-center gap-0 border-b border-border-light mb-4">
        {(['overview', 'runs', 'configuration'] as const).map(t => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 h-10 text-[0.8125rem] font-medium cursor-pointer transition-colors relative ${tab === t ? 'text-primary' : 'text-text-muted hover:text-text'}`}>
      ...
      <div className="flex-1 overflow-auto">
        {tab === 'overview' && (
          <div className="space-y-5 max-w-[800px]">
            <div>
              <h3 className="text-[0.8125rem] font-semibold text-text mb-2">Description</h3>
              <p className="text-[0.8125rem] text-text-muted leading-relaxed">{wf.description}</p>
            </div>
            {wf.steps && wf.steps.length > 0 && (
              <div>
                <h3 className="text-[0.8125rem] font-semibold text-text mb-3">Steps</h3>
                <div className="space-y-3">
                  {wf.steps.map((step, i) => (
                    <div key={i} className="flex items-start gap-3">
                      <div className="w-7 h-7 rounded-full bg-primary/10 text-primary text-[0.75rem] font-bold flex items-center justify-center shrink-0 mt-0.5">{i + 1}</div>
                      <div>
                        <div className="text-[0.8125rem] font-medium text-text">{step}</div>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div>
              <h3 className="text-[0.8125rem] font-semibold text-text mb-2">Tags</h3>
              <div className="flex flex-wrap gap-1.5">
                {wf.tags.map(t => <span key={t} className="inline-flex items-center px-2.5 py-1 rounded-md bg-surface-2 border border-border-light text-text-muted text-[0.75rem] font-medium">{t}</span>)}
              </div>
            </div>
          </div>
        )}
        {tab === 'runs' && (
          <div>
            {runs.length === 0 ? (
              <div className="text-center py-12 text-[0.8125rem] text-text-muted">No runs for this workflow yet.</div>
            ) : (
              <table className="w-full text-[0.75rem] border-collapse">
                <thead><tr className="border-b border-border-light text-[0.625rem] font-semibold uppercase tracking-wider text-text-muted">
                ...
                <tbody>{runs.map(r => (
                  <tr key={r.id} className="border-b border-border-light/60 hover:bg-surface-2/40">
                    ...
                    <td className="px-4 py-3 text-center"><span className={`px-2 py-0.5 rounded-full text-[0.5625rem] font-bold ${RUN_STATUS_CLS[r.status]}`}>{r.status}</span></td>
          ...
          <div className="space-y-5 max-w-[700px]">
            ...
            <div className="rounded-2xl border border-border-light bg-white p-5">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className="text-[0.75rem] text-text-muted">3 active</span>
              </div>
                ...
                {[
                  ...
                  <div key={rule.label} className="flex items-center justify-between px-4 py-3 rounded-xl border border-border-light bg-surface-2/30">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-[0.8125rem] font-semibold text-text">{rule.label}</div>
            ...
            <div className="rounded-2xl border border-border-light bg-white p-5 space-y-3">
              ...
              {[
                ...
                <div key={item.label} className="flex items-start gap-4 py-1.5 border-b border-border-light/40 last:border-b-0">
                  <span className="text-[0.75rem] text-text-muted w-[140px] shrink-0">{item.label}</span>
                  <span className="text-[0.8125rem] text-text font-medium">{item.value}</span>
                </div>
              ))}
            </div>
          </div>
