...
export default function AutomationOutputReviewTab({ engagement, runsState, onNavigateTab }: Props) {
  ...
  return (
    <div className="space-y-5">
      {/* Header */}
      <div>
        <h3 className="text-[15px] font-bold text-text mb-0.5">Bulk Execution Output</h3>
        <p className="text-[12px] text-text-muted">Workflow execution status and output summary.</p>
      </div>
      ...
      <div className="rounded-2xl border-2 border-dashed border-primary/20 bg-gradient-to-br from-primary/5 to-purple-50/30 p-8 text-center space-y-4">
        {runningRuns.length > 0 ? (
          <>
            ...
            <h4 className="text-[16px] font-bold text-text">Execution in Progress</h4>
          ...
          <>
            ...
            <h4 className="text-[16px] font-bold text-text">Bulk Execution Complete</h4>
            <p className="text-[12px] text-text-muted max-w-md mx-auto">
          ...
          <>
            ...
            <h4 className="text-[16px] font-bold text-text">Waiting for Execution</h4>
            <p className="text-[12px] text-text-muted max-w-md mx-auto">
              No workflows have been executed yet. Go to the Workflows tab to start a bulk run.
            </p>
            <button onClick={() => onNavigateTab?.('workflows')} className="px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-white text-[12px] font-semibold cursor-pointer transition-colors">
              Go to Workflows
            </button>
          </>
        )}
      ...
      {completedRuns.length > 0 && (
        <>
          <div className="grid grid-cols-4 gap-3">
            {[
              ...
              <div key={s.label} className="rounded-xl border border-border-light bg-white p-4 text-center">
                <div className={`text-[20px] font-bold tabular-nums ${s.cls || 'text-text'}`}>{s.value}</div>
          ...
          <div className="space-y-2">
            <h4 className="text-[12px] font-bold text-text">Workflow Outputs</h4>
              ...
              return (
                <div key={wfName} className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border-light bg-white">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-[13px] font-semibold text-text truncate">{wfName}</div>
