    ...
    return (
      <div className="space-y-4">
        ...
        <div className="rounded-xl border-2 border-canvas-border bg-canvas/30 p-6 text-center space-y-3">
          ...
          {warnings.map((w, i) => <div key={i} className="flex items-start gap-2 text-[0.625rem] text-amber-600 max-w-md mx-auto"><AlertCircle size={11} className="shrink-0 mt-0.5" /><span>{w}</span></div>)}
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-[0.9375rem] font-bold text-text mb-0.5">Schedule</h3>
          <p className="text-[0.75rem] text-text-muted">Configure recurring automation execution and monitoring.</p>
        </div>
        <span className={`px-2.5 py-1 rounded-full text-[0.625rem] font-bold shrink-0 ${SCHEDULE_STATUS_CLS[schedule.status]}`}>{schedule.status.replace(/_/g, ' ')}</span>
      </div>

      {/* Warnings */}
      {warnings.map((w, i) => (
        <div key={i} className="flex items-start gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200 text-[0.625rem] text-amber-700">
      ...
      <div className="flex items-center gap-3">
        {!isActive && !isPaused && <button onClick={saveDraft} className="px-3 py-1.5 rounded-lg border border-border-light text-[0.6875rem] font-medium text-text-muted hover:bg-surface-2/30 cursor-pointer transition-colors">Save Draft</button>}
        {!isActive && !isPaused && (
          <button onClick={activate} disabled={!canSchedule || !schedule.startDate || !schedule.runTime}
            className="px-4 py-2 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-[0.75rem] font-semibold cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed flex items-center gap-1.5">
            <Play size={12} />Activate Schedule
          </button>
        )}
        {isActive && <button onClick={pause} className="px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-[0.6875rem] font-semibold cursor-pointer transition-colors flex items-center gap-1.5"><Pause size={11} />Pause</button>}
        {isPaused && <button onClick={resume} className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-[0.6875rem] font-semibold cursor-pointer transition-colors flex items-center gap-1.5"><Play size={11} />Resume</button>}
        {(isActive || isPaused) && <button onClick={disable} className="px-3 py-1.5 rounded-lg border border-red-200 text-red-600 bg-red-50 text-[0.6875rem] font-semibold cursor-pointer transition-colors flex items-center gap-1.5"><Power size={11} />Disable</button>}
        {isActive && <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200 text-[0.625rem] text-emerald-700 flex-1"><CheckCircle2 size={11} className="shrink-0 mt-0.5" /><span>Schedule is active. Next run: {nextRun || '—'}. Real execution will be connected later.</span></div>}
      </div>

      {/* Notes */}
      <div className="rounded-lg border border-border-light p-4 space-y-2">
        <h4 className="text-[0.6875rem] font-bold text-text">Schedule Notes</h4>
  ...
  return (
    <div className="rounded-lg border border-border-light p-4 space-y-2">
      <div className="flex items-center justify-between">
        <h4 className="text-[0.6875rem] font-bold text-text">Automation Project Readiness</h4>
        <span className={`px-2 py-0.5 rounded-full text-[0.5625rem] font-bold ${allOk ? 'bg-emerald-50 text-emerald-700' : 'bg-amber-50 text-amber-700'}`}>{allOk ? 'Ready to Operate' : 'In Progress'}</span>
      </div>
