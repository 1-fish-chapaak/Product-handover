...
export default function AutomationRunsTab({ engagement, inputData, setup, runsState, onUpdateRuns, onNavigateTab }: Props) {
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div><h3 className="text-[0.9375rem] font-bold text-text mb-0.5">Runs</h3><p className="text-[0.75rem] text-text-muted">Execute selected workflows using the bulk run flow and review generated outputs and exceptions.</p></div>
      ...
      <div className="rounded-lg border-2 border-dashed border-primary/20 bg-primary/5 p-4 space-y-2">
        <div className="flex items-center justify-between">
          <h4 className="text-[0.75rem] font-bold text-text">{isWorkflowMode ? 'Execute Workflows' : 'Run Analysis'}</h4>
          {isBulk && <span className="px-2 py-0.5 rounded-full bg-purple-50 text-purple-700 text-[0.5625rem] font-bold">{wfNames.length} workflows selected</span>}
        </div>
        {isBulk && (
        ...
        {isWorkflowMode ? (
          <div className="flex items-center gap-3">
            <input value={runName} onChange={e => setRunName(e.target.value)} placeholder={defaultRunName} className="flex-1 px-3 py-2 border border-border rounded-lg text-[0.75rem] text-text bg-white outline-none focus:border-primary/40" />
            <button onClick={() => setShowBulkModal(true)}
              className="px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-white text-[0.75rem] font-semibold cursor-pointer transition-colors shrink-0 flex items-center gap-1.5">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={handleCreateQARun}
              className="px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-white text-[0.75rem] font-semibold cursor-pointer transition-colors shrink-0">Run Q&A Analysis</button>
          </div>
        )}
      </div>
