    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <span key={s.label} className="px-2.5 py-1 rounded-lg bg-canvas border border-border-light text-[0.625rem] text-text font-medium">
      ...
      <div className="rounded-lg border border-border-light bg-white p-5 space-y-4">
        ...
        <Section num={1} title="Working Paper Header">
          <div className="grid grid-cols-3 gap-3 text-[0.6875rem]">
            <div><span className="text-ink-400 block text-[0.625rem]">Assignment</span><span className="text-text font-medium">{engagement.name}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Audit Period</span><span className="text-text font-medium">{cfg.auditPeriodStart || '—'} to {cfg.auditPeriodEnd || '—'}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Entity</span><span className="text-text font-medium">{engagement.entityOrLocation || '—'}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Owner</span><span className="text-text font-medium">{engagement.owner}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Reviewer</span><span className="text-text font-medium">{engagement.reviewer || '—'}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Process Owner</span><span className="text-text font-medium">{cfg.processOwner || '—'}</span></div>
          </div>
        </Section>

        {/* 2. Scope */}
        <Section num={2} title="Audit Scope Summary">
          <div className="grid grid-cols-3 gap-3 text-[0.6875rem] mb-2">
            <div><span className="text-ink-400 block text-[0.625rem]">Scope Level</span><span className="text-text font-medium">{SCOPE_LEVEL_LABELS[scope.scopeLevel]?.label || scope.scopeLevel}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Business Process</span><span className="text-text font-medium">{selectedBP ? `${selectedBP.code} — ${selectedBP.name}` : '—'}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Readiness</span><span className={`font-medium ${scopeReady ? 'text-emerald-600' : 'text-amber-600'}`}>{scopeReadiness.status}</span></div>
          </div>
          {scope.scopeObjective && <div className="text-[0.6875rem] mb-1"><span className="text-ink-400">Objective: </span><span className="text-text">{scope.scopeObjective}</span></div>}
          {scope.inScopeItems && <div className="text-[0.6875rem] mb-1"><span className="text-ink-400">In Scope: </span><span className="text-text">{scope.inScopeItems}</span></div>}
          {scope.outOfScopeItems && <div className="text-[0.6875rem]"><span className="text-ink-400">Out of Scope: </span><span className="text-text">{scope.outOfScopeItems}</span></div>}
          {scope.sopIds.length > 0 && <div className="flex flex-wrap gap-1 mt-1">{scope.sopIds.map(id => { const s = SOPS.find(x => x.id === id); return s ? <span key={id} className="px-1.5 py-0.5 rounded bg-primary/10 text-primary text-[0.5rem] font-semibold">{s.name}</span> : null; })}</div>}
        </Section>

        {/* 3. Announcement */}
        <Section num={3} title="Announcement Summary">
          <div className="grid grid-cols-3 gap-3 text-[0.6875rem]">
            <div><span className="text-ink-400 block text-[0.625rem]">Status</span><span className={`font-medium ${announcementDone ? 'text-emerald-600' : 'text-amber-600'}`}>{announcement.status}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Subject</span><span className="text-text font-medium">{announcement.subject || '—'}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Recipients</span><span className="text-text font-medium">{announcement.recipients || '—'}</span></div>
          </div>
          {announcement.sentAt && <div className="text-[0.625rem] text-ink-500 mt-1">Sent: {announcement.sentAt}{announcement.acknowledgedAt ? ` · Acknowledged: ${announcement.acknowledgedAt}` : ''}</div>}
          {!announcementDone && <div className="flex items-start gap-2 mt-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200 text-[0.625rem] text-amber-700"><AlertCircle size={11} className="shrink-0 mt-0.5" /><span>Announcement has not been marked as sent.</span></div>}
        </Section>

        {/* 4. IDR */}
        <Section num={4} title="IDR / Request Summary">
          <div className="grid grid-cols-4 gap-3 text-[0.6875rem] mb-2">
            <div><span className="text-ink-400 block text-[0.625rem]">Total</span><span className="text-text font-medium">{reqSummary.total}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Received</span><span className="text-emerald-600 font-medium">{reqSummary.received}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Pending</span><span className={`font-medium ${reqSummary.pending > 0 ? 'text-amber-600' : 'text-ink-400'}`}>{reqSummary.pending}</span></div>
            <div><span className="text-ink-400 block text-[0.625rem]">Overdue</span><span className={`font-medium ${reqSummary.overdue > 0 ? 'text-red-600' : 'text-ink-400'}`}>{reqSummary.overdue}</span></div>
          </div>
          {requests.proceedWithoutIDR && <div className="text-[0.625rem] text-blue-600 flex items-center gap-1"><Info size={10} />Analysis proceeded without IDR.</div>}
              ...
              <table className="w-full text-[0.625rem]">
                ...
                <tbody>{requests.requests.map(r => (
                  <tr key={r.id} className="border-b border-border-light/50">
                    ...
                    <td className="px-2 py-1.5 text-ink-500">{REQUEST_TYPE_LABELS[r.requestType]}</td>
                    <td className="px-2 py-1.5 text-ink-500">{r.requestedFrom}</td>
                    <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${r.status === 'RECEIVED' ? 'bg-emerald-50 text-emerald-700' : r.status === 'OVERDUE' ? 'bg-red-50 text-red-700' : 'bg-canvas text-ink-500'}`}>{r.status.replace(/_/g, ' ')}</span></td>
        ...
        <Section num={5} title="Analysis Procedures and Outputs">
          {analysis.runs.length === 0 ? (
            <div className="text-[0.6875rem] text-ink-400 italic">No analysis runs completed yet.</div>
          ) : (
                ...
                <tbody>{analysis.runs.map(run => (
                  <React.Fragment key={run.id}>
                    <tr className={`border-b border-border-light/50 cursor-pointer hover:bg-surface-2/20 ${expandedRunId === run.id ? 'bg-surface-2/20' : ''}`} onClick={() => setExpandedRunId(expandedRunId === run.id ? null : run.id)}>
                      <td className="px-2 py-1.5 text-ink-400">{expandedRunId === run.id ? <ChevronDown size={9} /> : <ChevronRight size={9} />}</td>
                      <td className="px-2 py-1.5 text-text font-medium">{run.title}</td>
                      <td className="px-2 py-1.5 text-center text-ink-500">{RUN_TYPE_LABELS[run.runType]}</td>
                      <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${run.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700' : 'bg-canvas text-ink-500'}`}>{run.status}</span></td>
                      <td className="px-2 py-1.5 text-center">{run.exceptions.length}</td>
                      <td className="px-2 py-1.5 text-ink-500 truncate max-w-[200px]">{run.summary || '—'}</td>
                    </tr>
                    {expandedRunId === run.id && run.status === 'COMPLETED' && (
                      <tr><td colSpan={6} className="p-0">
                        <div className="bg-surface-2/15 px-4 py-2 border-b border-border-light text-[0.5625rem] space-y-1">
                          <div><span className="text-ink-400">Input files:</span> <span className="text-text">{run.inputFiles.join(', ') || '—'}</span></div>
                          <div><span className="text-ink-400">{run.workflowName ? 'Workflow:' : 'Question:'}</span> <span className="text-text">{run.workflowName || run.question || '—'}</span></div>
                          <div><span className="text-ink-400">Completed:</span> <span className="text-text">{run.completedAt} by {run.runBy}</span></div>
                        </div>
                      </td></tr>
                    )}
                  </React.Fragment>
        ...
        <Section num={6} title={`Exceptions Identified (${allExceptions.length})`}>
          {allExceptions.length === 0 ? (
            <div className="text-[0.6875rem] text-ink-400 italic flex items-center gap-2"><CheckCircle2 size={12} className="text-emerald-500" />No exceptions identified.</div>
          ) : (
            <div className="rounded-lg border border-border-light overflow-hidden">
              <table className="w-full text-[0.625rem]">
                <thead><tr className="border-b border-border-light bg-surface-2/30 text-[0.5rem] font-semibold text-ink-400 uppercase">
                ...
                <tbody>{allExceptions.map(ex => (
                  <tr key={ex.id} className="border-b border-border-light/50">
                    ...
                    <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${EX_SEVERITY_CLS[ex.severity]}`}>{ex.severity}</span></td>
                    <td className="px-2 py-1.5 text-ink-500">{ex.source}</td>
                    <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${EX_STATUS_CLS[ex.status]}`}>{ex.status.replace(/_/g, ' ')}</span></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
        ...
        <Section num={7} title="Observations Summary">
          {observations.noObservationsConfirmed ? (
            <div className="flex items-center gap-2 text-[0.6875rem] text-emerald-600"><CheckCircle2 size={12} />No observations noted has been confirmed.</div>
          ) : activeObs.length === 0 ? (
            <div className="flex items-start gap-2 text-[0.6875rem] text-amber-600"><AlertCircle size={12} className="shrink-0 mt-0.5" />Observation section is not complete.</div>
          ) : (
            <div className="rounded-lg border border-border-light overflow-hidden">
              <table className="w-full text-[0.625rem]">
                <thead><tr className="border-b border-border-light bg-surface-2/30 text-[0.5rem] font-semibold text-ink-400 uppercase">
                ...
                <tbody>{activeObs.map(o => (
                  <tr key={o.id} className="border-b border-border-light/50">
                    ...
                    <td className="px-2 py-1.5 text-center text-ink-500">{CATEGORY_LABELS[o.observationCategory]}</td>
                    <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${OBS_SEVERITY_CLS[o.severity]}`}>{o.severity}</span></td>
                    <td className="px-2 py-1.5 text-ink-500">{o.processOwner || '—'}</td>
                    <td className="px-2 py-1.5 text-center font-mono text-ink-500">{o.targetRemediationDate || '—'}</td>
                    <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${OBS_STATUS_CLS[o.status]}`}>{o.status.replace(/_/g, ' ')}</span></td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
        ...
        <Section num={8} title="Discussion / Management Response">
          {discussion.noObsDiscussionConfirmed ? (
            <div className="flex items-center gap-2 text-[0.6875rem] text-emerald-600"><CheckCircle2 size={12} />Discussion closed with no observations.</div>
          ) : discussion.items.length === 0 ? (
            <div className="text-[0.6875rem] text-amber-600 flex items-start gap-2"><AlertCircle size={12} className="shrink-0 mt-0.5" />Management discussion is not fully complete.</div>
          ) : (
            <div className="rounded-lg border border-border-light overflow-hidden">
              <table className="w-full text-[0.625rem]">
                <thead><tr className="border-b border-border-light bg-surface-2/30 text-[0.5rem] font-semibold text-ink-400 uppercase">
                ...
                <tbody>{discussion.items.map(d => (
                  <tr key={d.id} className="border-b border-border-light/50">
                    ...
                    <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${DISC_STATUS_CLS[d.status]}`}>{d.status.replace(/_/g, ' ')}</span></td>
                    <td className="px-2 py-1.5 text-ink-500 truncate max-w-[120px]">{d.managementResponse || '—'}</td>
                    <td className="px-2 py-1.5 text-ink-500 truncate max-w-[120px]">{d.agreedAction || '—'}</td>
                    <td className="px-2 py-1.5 text-ink-500">{d.actionOwner || '—'}</td>
                    <td className="px-2 py-1.5 text-center font-mono text-ink-500">{d.targetDate || '—'}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
          ...
          {discussion.notes.length > 0 && (
            <div className="mt-2"><h6 className="text-[0.5625rem] font-bold text-ink-400 uppercase tracking-wider mb-1">Discussion Notes</h6>
              <div className="space-y-1">{discussion.notes.map(n => <div key={n.id} className="text-[0.625rem] text-ink-500 pl-2 border-l-2 border-canvas-border">{n.note} — {n.author}, {n.createdAt}</div>)}</div>
            </div>
          )}
        </Section>
        ...
        <Section num={9} title="Evidence / Source File Index">
          {sourceFiles.length === 0 ? (
            <div className="text-[0.6875rem] text-ink-400 italic">No source files received.</div>
          ) : (
            <div className="rounded-lg border border-border-light overflow-hidden">
              <table className="w-full text-[0.625rem]">
                <thead><tr className="border-b border-border-light bg-surface-2/30 text-[0.5rem] font-semibold text-ink-400 uppercase">
                ...
                <tbody>{sourceFiles.map((sf, i) => (
                  <tr key={i} className="border-b border-border-light/50">
                    <td className="px-2 py-1.5 text-text flex items-center gap-1"><FileText size={9} className="text-ink-400 shrink-0" />{sf.file}</td>
                    <td className="px-2 py-1.5 text-ink-500">{sf.sourceType}</td>
                    <td className="px-2 py-1.5 text-ink-500">{sf.source}</td>
                    <td className="px-2 py-1.5 text-ink-500">{sf.scope}</td>
                    <td className="px-2 py-1.5 text-center">{sf.usedInAnalysis ? <CheckCircle2 size={10} className="text-emerald-500 mx-auto" /> : <span className="text-ink-300">—</span>}</td>
                  </tr>
                ))}</tbody>
              </table>
            </div>
        ...
        <Section num={10} title="Working Paper Readiness">
          ...
          <div className="mt-2 flex items-center gap-2">
            <span className={`px-2 py-0.5 rounded-full text-[0.5625rem] font-bold ${wpReady ? 'bg-emerald-50 text-emerald-700' : analysisHasRuns || obsDone ? 'bg-amber-50 text-amber-700' : 'bg-canvas text-ink-500'}`}>
        ...
        <div>
          ...
          <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-canvas text-[0.6875rem] text-ink-500">
