    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2">
        {[
          ...
          <span key={s.label} className="px-2.5 py-1 rounded-lg bg-gray-50 border border-border-light text-[0.625rem] text-text font-medium">
      ...
      <div className="rounded-xl border border-border-light bg-white p-5 space-y-4">
        ...
        <Section num={1} title="Working Paper Header">
          <div className="grid grid-cols-3 gap-3 text-[0.6875rem]">
            <div><span className="text-gray-400 block text-[0.625rem]">Assignment</span><span className="text-text font-medium">{engagement.name}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Audit Period</span><span className="text-text font-medium">{cfg.auditPeriodStart || '—'} to {cfg.auditPeriodEnd || '—'}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Entity</span><span className="text-text font-medium">{engagement.entityOrLocation || '—'}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Owner</span><span className="text-text font-medium">{engagement.owner}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Reviewer</span><span className="text-text font-medium">{engagement.reviewer || '—'}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Process Owner</span><span className="text-text font-medium">{cfg.processOwner || '—'}</span></div>
          </div>
        </Section>
        ...
        <Section num={2} title="Audit Scope Summary">
          <div className="grid grid-cols-3 gap-3 text-[0.6875rem] mb-2">
            <div><span className="text-gray-400 block text-[0.625rem]">Scope Level</span><span className="text-text font-medium">{SCOPE_LEVEL_LABELS[scope.scopeLevel]?.label || scope.scopeLevel}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Business Process</span><span className="text-text font-medium">{selectedBP ? `${selectedBP.code} — ${selectedBP.name}` : '—'}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Readiness</span><span className={`font-medium ${scopeReady ? 'text-emerald-600' : 'text-amber-600'}`}>{scopeReadiness.status}</span></div>
          </div>
          {scope.scopeObjective && <div className="text-[0.6875rem] mb-1"><span className="text-gray-400">Objective: </span><span className="text-text">{scope.scopeObjective}</span></div>}
          {scope.inScopeItems && <div className="text-[0.6875rem] mb-1"><span className="text-gray-400">In Scope: </span><span className="text-text">{scope.inScopeItems}</span></div>}
          {scope.outOfScopeItems && <div className="text-[0.6875rem]"><span className="text-gray-400">Out of Scope: </span><span className="text-text">{scope.outOfScopeItems}</span></div>}
        ...
        <Section num={3} title="Announcement Summary">
          <div className="grid grid-cols-3 gap-3 text-[0.6875rem]">
            <div><span className="text-gray-400 block text-[0.625rem]">Status</span><span className={`font-medium ${announcementDone ? 'text-emerald-600' : 'text-amber-600'}`}>{announcement.status}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Subject</span><span className="text-text font-medium">{announcement.subject || '—'}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Recipients</span><span className="text-text font-medium">{announcement.recipients || '—'}</span></div>
          </div>
          {announcement.sentAt && <div className="text-[0.625rem] text-gray-500 mt-1">Sent: {announcement.sentAt}{announcement.acknowledgedAt ? ` · Acknowledged: ${announcement.acknowledgedAt}` : ''}</div>}
        ...
        <Section num={4} title="IDR / Request Summary">
          <div className="grid grid-cols-4 gap-3 text-[0.6875rem] mb-2">
            <div><span className="text-gray-400 block text-[0.625rem]">Total</span><span className="text-text font-medium">{reqSummary.total}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Received</span><span className="text-emerald-600 font-medium">{reqSummary.received}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Pending</span><span className={`font-medium ${reqSummary.pending > 0 ? 'text-amber-600' : 'text-gray-400'}`}>{reqSummary.pending}</span></div>
            <div><span className="text-gray-400 block text-[0.625rem]">Overdue</span><span className={`font-medium ${reqSummary.overdue > 0 ? 'text-red-600' : 'text-gray-400'}`}>{reqSummary.overdue}</span></div>
          </div>
              ...
              <table className="w-full text-[0.625rem]">
                ...
                <tbody>{requests.requests.map(r => (
                  <tr key={r.id} className="border-b border-border-light/50">
                    ...
                    <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${r.status === 'RECEIVED' ? 'bg-emerald-50 text-emerald-700' : r.status === 'OVERDUE' ? 'bg-red-50 text-red-700' : 'bg-gray-100 text-gray-500'}`}>{r.status.replace(/_/g, ' ')}</span></td>
        ...
        <Section num={5} title="Analysis Procedures and Outputs">
          {analysis.runs.length === 0 ? (
            <div className="text-[0.6875rem] text-gray-400 italic">No analysis runs completed yet.</div>
          ) : (
                ...
                <tbody>{analysis.runs.map(run => (
                  <React.Fragment key={run.id}>
                    <tr className={`border-b border-border-light/50 cursor-pointer hover:bg-surface-2/20 ${expandedRunId === run.id ? 'bg-surface-2/20' : ''}`} onClick={() => setExpandedRunId(expandedRunId === run.id ? null : run.id)}>
                      ...
                      <td className="px-2 py-1.5 text-center"><span className={`px-1.5 py-0.5 rounded text-[0.4375rem] font-bold ${run.status === 'COMPLETED' ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-500'}`}>{run.status}</span></td>
        ...
        <Section num={6} title={`Exceptions Identified (${allExceptions.length})`}>
          {allExceptions.length === 0 ? (
            <div className="text-[0.6875rem] text-gray-400 italic flex items-center gap-2"><CheckCircle2 size={12} className="text-emerald-500" />No exceptions identified.</div>
          ) : (
            <div className="rounded-lg border border-border-light overflow-hidden">
              <table className="w-full text-[0.625rem]">
                <thead><tr className="border-b border-border-light bg-surface-2/30 text-[0.5rem] font-semibold text-gray-400 uppercase">
        ...
        <Section num={8} title="Discussion / Management Response">
          ...
          {discussion.notes.length > 0 && (
            <div className="mt-2"><h6 className="text-[0.5625rem] font-bold text-gray-400 uppercase tracking-wider mb-1">Discussion Notes</h6>
              <div className="space-y-1">{discussion.notes.map(n => <div key={n.id} className="text-[0.625rem] text-gray-500 pl-2 border-l-2 border-gray-200">{n.note} — {n.author}, {n.createdAt}</div>)}</div>
            </div>
          )}
        </Section>
        ...
        <Section num={9} title="Evidence / Source File Index">
          {sourceFiles.length === 0 ? (
            <div className="text-[0.6875rem] text-gray-400 italic">No source files received.</div>
          ) : (
            <div className="rounded-lg border border-border-light overflow-hidden">
              <table className="w-full text-[0.625rem]">
                <thead><tr className="border-b border-border-light bg-surface-2/30 text-[0.5rem] font-semibold text-gray-400 uppercase">
        ...
        <Section num={10} title="Working Paper Readiness">
          ...
          <div className="mt-2 flex items-center gap-2">
            <span className={`px-2 py-0.5 rounded-full text-[0.5625rem] font-bold ${wpReady ? 'bg-emerald-50 text-emerald-700' : analysisHasRuns || obsDone ? 'bg-amber-50 text-amber-700' : 'bg-gray-100 text-gray-500'}`}>
        ...
        <div>
          ...
          <div className="flex items-start gap-2 px-3 py-2 rounded-lg bg-gray-50 text-[0.6875rem] text-gray-500">
