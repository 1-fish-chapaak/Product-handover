  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-[0.9375rem] font-bold text-text mb-0.5">Action Plan</h3>
          <p className="text-[0.75rem] text-text-muted">Track agreed remediation actions from the final internal audit report.</p>
        </div>
      ...
      {noActionsNeeded && (
        <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200 text-[0.6875rem] text-emerald-700">
              ...
              <tr key={item.id} className="border-b border-border-light/50">
                ...
                <td className="px-3 py-2 text-center">
                  <div className="flex items-center justify-center gap-1">
                    ...
                    {item.status === 'NOT_STARTED' && <button onClick={() => transitionStatus(item.id, 'IN_PROGRESS')} className="px-2 py-1 rounded text-[0.5rem] font-semibold text-blue-600 bg-blue-50 hover:bg-blue-100 cursor-pointer transition-colors">Start</button>}
                    {item.status === 'IN_PROGRESS' && <button onClick={() => transitionStatus(item.id, 'COMPLETED')} className="px-2 py-1 rounded text-[0.5rem] font-semibold text-emerald-600 bg-emerald-50 hover:bg-emerald-100 cursor-pointer transition-colors">Done</button>}
                  </div>
                </td>
              </tr>
            ))}</tbody>
      ...
      <div className="rounded-lg border border-border-light p-4 space-y-2">
        <h4 className="text-[0.6875rem] font-bold text-text">Follow-up</h4>
        ...
        {actionPlan.followUpRequired && (
          <>
            ...
            <button onClick={() => alert('Linked follow-up assignment creation will be connected later.')}
              className="text-[0.625rem] font-semibold text-primary hover:underline cursor-pointer">Create Linked Follow-up Assignment</button>
          </>
        )}
      </div>
    ...
    <div className="rounded-lg border-2 border-primary/20 bg-primary/5 p-4 space-y-3">
      ...
      <div className="flex items-center justify-end gap-2">
        ...
        <button onClick={() => { if (!title.trim()) return; onSave({ id: `ap-${Date.now()}`, sourceObservationId: '', sourceObservationTitle: '', title: title.trim(), description: desc.trim(), owner: owner.trim(), priority, dueDate, status: 'NOT_STARTED', remediationEvidence: [], comments: '', createdAt: new Date().toISOString().slice(0, 10), updatedAt: new Date().toISOString().slice(0, 10), completedAt: null, history: [{ id: `aph-${Date.now()}`, action: 'CREATED', actor: engagement.owner, timestamp: now(), comments: 'Manual action.' }] }); }}
          disabled={!title.trim()} className="px-4 py-1.5 rounded-lg bg-primary hover:bg-primary/90 text-white text-[0.6875rem] font-semibold cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Create</button>
      </div>
    </div>
  );
}
    ...
    <div className="rounded-xl border-2 border-primary/20 bg-white p-4 space-y-3 shadow-lg">
      ...
      <div>
        ...
        {item.remediationEvidence.length > 0 && (
          <div className="flex flex-wrap gap-1 mb-2">{item.remediationEvidence.map((f, i) => (
            <span key={i} className="flex items-center gap-1 px-2 py-0.5 rounded bg-emerald-50 border border-emerald-200 text-[0.5625rem] text-emerald-700"><FileText size={8} />{f}</span>
          ))}</div>
        )}
        <div className="flex items-center gap-2">
          <input value={newEvidence} onChange={e => setNewEvidence(e.target.value)} placeholder="Evidence file name..." className="flex-1 px-3 py-1.5 border border-border rounded-lg text-[0.6875rem] text-text bg-white outline-none focus:border-primary/40" />
          <button onClick={() => { if (newEvidence.trim()) { onAddEvidence(newEvidence.trim()); setNewEvidence(''); } }} disabled={!newEvidence.trim()}
            className="px-3 py-1.5 rounded-lg bg-primary/10 text-primary text-[0.625rem] font-semibold hover:bg-primary/20 cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Add</button>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button onClick={() => { onUpdate({ title: draft.title, description: draft.description, owner: draft.owner, priority: draft.priority, dueDate: draft.dueDate, comments: draft.comments, history: [...draft.history, { id: `aph-${Date.now()}`, action: 'UPDATED', actor: 'Auditor', timestamp: now(), comments: '' }] }); onClose(); }}
          className="px-3 py-1.5 rounded-lg bg-primary hover:bg-primary/90 text-white text-[0.6875rem] font-semibold cursor-pointer transition-colors">Save</button>
        {item.status === 'NOT_STARTED' && <button onClick={() => onTransition('IN_PROGRESS')} className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[0.6875rem] font-semibold cursor-pointer transition-colors flex items-center gap-1"><Play size={10} />Start</button>}
        {item.status === 'IN_PROGRESS' && <button onClick={() => onTransition('COMPLETED')} className="px-3 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-700 text-white text-[0.6875rem] font-semibold cursor-pointer transition-colors flex items-center gap-1"><CheckCircle2 size={10} />Complete</button>}
        {item.status === 'IN_PROGRESS' && <button onClick={() => onTransition('DEFERRED')} className="px-3 py-1.5 rounded-lg border border-amber-200 text-amber-700 bg-amber-50 text-[0.6875rem] font-semibold cursor-pointer transition-colors">Defer</button>}
        {item.status === 'DEFERRED' && <button onClick={() => onTransition('IN_PROGRESS')} className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-700 text-white text-[0.6875rem] font-semibold cursor-pointer transition-colors">Resume</button>}
