  ...
  if (!hasDraft) {
    return (
      <div className="space-y-4">
        <div><h3 className="text-[15px] font-bold text-text mb-0.5">Final Report</h3><p className="text-[12px] text-text-muted">Generate the formal internal audit report for process owner review and closure.</p></div>
        <div className="rounded-xl border-2 border-dashed border-primary/20 bg-gradient-to-br from-primary/5 to-purple-50/30 p-8 text-center space-y-4">
          ...
          <h4 className="text-[16px] font-bold text-text">Generate Internal Audit Report</h4>
          <p className="text-[12px] text-text-muted max-w-lg mx-auto">
          ...
          {activeObs.length === 0 && !iaState.observations.noObservationsConfirmed && (
            <div className="flex items-center justify-center gap-2 text-[11px] text-amber-600"><AlertCircle size={12} />No observations ready. Complete observations and discussion first.</div>
          )}
          <button onClick={handleGenerate} className="px-5 py-2.5 rounded-lg bg-gradient-to-r from-primary to-primary-medium text-white text-[13px] font-semibold hover:from-primary-hover hover:to-primary cursor-pointer transition-all">
    ...
    <div className="space-y-5">
      ...
      <div className="relative rounded-2xl overflow-hidden bg-gradient-to-br from-[#3b0b72] to-[#6a12cd]" style={{ boxShadow: '0 4px 24px rgba(106,18,205,0.35)' }}>
        ...
        <div className="relative z-10 px-8 py-7">
          <p className="text-white/50 text-[11px] font-semibold uppercase tracking-wider mb-1">Internal Audit Report</p>
          ...
          <p className="text-white/60 text-[13px] mb-3">{engagement.description || `${bp?.name || ''} process audit — ${engagement.entityOrLocation || ''}`}</p>
          <div className="flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              <span className={`px-2.5 py-1 rounded-[10px] text-[11px] font-bold ${isIssued ? 'bg-white/20 text-white' : 'bg-white/10 text-white/80'}`}>{finalReport.status.replace(/_/g, ' ')}</span>
              {!isIssued && (
                <button onClick={handleGenerate} className="inline-flex items-center gap-1.5 h-9 px-3.5 text-[12.5px] font-semibold text-primary bg-white rounded-[10px] hover:bg-white/90 transition-colors cursor-pointer shadow-[0_2px_8px_rgba(0,0,0,0.15)]">
      ...
      <div className="bg-white rounded-xl border border-border-light p-5">
        <div className="grid grid-cols-3 gap-x-8 gap-y-3 text-[12px]">
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Assignment</span><span className="text-text font-semibold">{engagement.name}</span></div>
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Business Process</span><span className="text-text font-semibold">{bp?.name || engagement.businessProcess || '—'}</span></div>
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Entity / Location</span><span className="text-text font-semibold">{engagement.entityOrLocation || '—'}</span></div>
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Audit Period</span><span className="text-text font-semibold">{engagement.dataPeriodStart || '—'} to {engagement.dataPeriodEnd || '—'}</span></div>
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Prepared By</span><span className="text-text font-semibold">{finalReport.preparedBy || engagement.owner}</span></div>
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Reviewed By</span><span className="text-text font-semibold">{finalReport.reviewedBy || engagement.reviewer || '—'}</span></div>
        </div>
      </div>
        ...
        <div className="grid grid-cols-4 gap-3 mb-4">
          {[
            ...
            <div key={stat.label} className="bg-white rounded-xl border border-border-light p-4 flex items-center gap-3">
              ...
              <div><div className="text-xl font-bold text-text">{stat.value}</div><div className="text-[10px] text-text-muted">{stat.label}</div></div>
            </div>
          ))}
        </div>
        <p className="text-[12px] text-text-secondary leading-relaxed">{finalReport.executiveSummary}</p>
      </div>

      {/* ══ Scope and Objective ══ */}
      <div className="bg-white rounded-xl border border-border-light p-5">
        <h3 className="text-[13px] font-bold text-text mb-2 flex items-center gap-2"><Shield size={14} className="text-primary" /> Scope and Objective</h3>
        <p className="text-[12px] text-text-secondary leading-relaxed mb-3">{finalReport.scopeAndObjective}</p>
        {(iaState.scope.sopIds.length > 0 || iaState.scope.racmVersionIds.length > 0 || iaState.scope.checklistIds.length > 0) && (
          <div className="flex flex-wrap gap-1.5 mt-2">
            {iaState.scope.sopIds.map(id => { const s = SOPS.find(x => x.id === id); return s ? <span key={id} className="px-2 py-0.5 rounded bg-amber-50 text-amber-700 text-[9px] font-medium">SOP: {s.name}</span> : null; })}
            {iaState.scope.racmVersionIds.map(id => { const r = RACMS.find(x => x.id === id); return r ? <span key={id} className="px-2 py-0.5 rounded bg-purple-50 text-purple-700 text-[9px] font-medium">RACM: {r.name}</span> : null; })}
            {iaState.scope.checklistIds.map(id => { const c = CHECKLISTS.find(x => x.id === id); return c ? <span key={id} className="px-2 py-0.5 rounded bg-blue-50 text-blue-700 text-[9px] font-medium">Checklist: {c.name}</span> : null; })}
          </div>
        )}
      </div>
      ...
      {/* ══ Audit Approach ══ */}
      <div className="bg-white rounded-xl border border-border-light p-5">
        <h3 className="text-[13px] font-bold text-text mb-2 flex items-center gap-2"><BarChart3 size={14} className="text-primary" /> Audit Approach</h3>
        <p className="text-[12px] text-text-secondary leading-relaxed mb-2">{finalReport.proceduresPerformed}</p>
        {receivedFiles.length > 0 && <p className="text-[11px] text-gray-400">{finalReport.dataReviewed}</p>}
      </div>

      {/* ══ Key Metrics ══ */}
      <div className="bg-white rounded-xl border border-border-light p-5">
        <h3 className="text-[13px] font-bold text-text mb-3 flex items-center gap-2"><BarChart3 size={14} className="text-primary" /> Audit Snapshot</h3>
      ...
      <div className="bg-white rounded-xl border border-border-light p-5">
        <h3 className="text-[13px] font-bold text-text mb-3 flex items-center gap-2"><AlertTriangle size={14} className="text-amber-500" /> Detailed Observations ({activeObs.length})</h3>
        {iaState.observations.noObservationsConfirmed ? (
          <div className="text-[12px] text-emerald-600 flex items-center gap-2 py-3"><CheckCircle2 size={13} />No audit observations were noted during this assignment.</div>
        ) : activeObs.length === 0 ? (
          <div className="text-[12px] text-gray-400 italic py-3">Observations pending formalization.</div>
        ) : (
                  ...
                  <div className="px-4 py-3 bg-surface-2/20 border-b border-border-light">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[13px] font-semibold text-text">{obs.title}</span>
                        <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${OBS_SEV_CLS[obs.severity]}`}>{obs.severity}</span>
                      </div>
                      <span className="text-[10px] text-gray-400">{CATEGORY_LABELS[obs.observationCategory]}</span>
                    </div>
                  </div>
                  <div className="px-4 py-3 space-y-2 text-[11px]">
                    <div className="grid grid-cols-2 gap-x-6 gap-y-2">
                      <div><span className="text-gray-400 block text-[9px] font-medium">Control / Check</span><span className="text-text">{obs.linkedControlName || obs.linkedScopeLabel || '—'}</span></div>
                      <div><span className="text-gray-400 block text-[9px] font-medium">Source</span><span className="text-text">{obs.sourceType === 'ANALYSIS_EXCEPTION' ? `Analysis — ${wfRun?.workflowName || 'Workflow'}` : 'Manual'}</span></div>
                      {obs.description && <div className="col-span-2"><span className="text-gray-400 block text-[9px] font-medium">Condition (What was found)</span><span className="text-text">{obs.description}</span></div>}
                      {obs.rootCause && <div><span className="text-gray-400 block text-[9px] font-medium">Root Cause</span><span className="text-text">{obs.rootCause}</span></div>}
                      {obs.impact && <div><span className="text-gray-400 block text-[9px] font-medium">Impact / Risk</span><span className="text-text">{obs.impact}</span></div>}
                      {obs.recommendation && <div className="col-span-2"><span className="text-gray-400 block text-[9px] font-medium">Recommendation</span><span className="text-text">{obs.recommendation}</span></div>}
                    </div>
                    {disc && (
                      <div className="border-t border-border-light/50 pt-2 mt-2 grid grid-cols-2 gap-x-6 gap-y-2">
                        <div className="col-span-2"><span className="text-gray-400 block text-[9px] font-medium">Process Owner Response</span><span className="text-text">{disc.managementResponse || '—'}</span></div>
                        {disc.agreedAction && <div><span className="text-gray-400 block text-[9px] font-medium">Agreed Action</span><span className="text-text">{disc.agreedAction}</span></div>}
                        <div><span className="text-gray-400 block text-[9px] font-medium">Action Owner</span><span className="text-text">{disc.actionOwner || obs.processOwner || '—'}</span></div>
                        <div><span className="text-gray-400 block text-[9px] font-medium">Target Date</span><span className="text-text font-mono">{disc.targetDate || obs.targetRemediationDate || '—'}</span></div>
                        <div><span className="text-gray-400 block text-[9px] font-medium">Discussion Status</span><span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${disc.status === 'AGREED' || disc.status === 'READY_FOR_REPORT' ? 'bg-emerald-50 text-emerald-700' : disc.status === 'DISAGREED' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>{disc.status.replace(/_/g, ' ')}</span></div>
                      </div>
                    )}
                  </div>
                </div>
      ...
      {iaState.discussion.items.length > 0 && (
        <div className="bg-white rounded-xl border border-border-light p-5">
          <h3 className="text-[13px] font-bold text-text mb-3 flex items-center gap-2"><Users size={14} className="text-primary" /> Process Owner Response Summary</h3>
            ...
            <table className="w-full text-[11px]">
              ...
              <tbody>{iaState.discussion.items.map(d => (
                <tr key={d.id} className="border-b border-border-light/50">
                  ...
                  <td className="px-3 py-2 text-center"><span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${d.status === 'AGREED' || d.status === 'READY_FOR_REPORT' ? 'bg-emerald-50 text-emerald-700' : d.status === 'DISAGREED' ? 'bg-red-50 text-red-700' : 'bg-amber-50 text-amber-700'}`}>{d.status.replace(/_/g, ' ')}</span></td>
                  ...
                  <td className="px-3 py-2 text-center">{d.remediationRequired ? <span className="text-[9px] font-semibold text-amber-700">Yes</span> : <span className="text-[9px] text-gray-400">No</span>}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
      ...
      <div className={`rounded-xl border-2 p-5 ${RATING_CLS[finalReport.overallRating]}`}>
        <h3 className="text-[14px] font-bold mb-2">Overall Audit Conclusion: {RATING_LABELS[finalReport.overallRating]}</h3>
        <p className="text-[12px] leading-relaxed opacity-80">{finalReport.conclusionRemarks}</p>
      </div>

      {/* ══ Sign-off / Distribution ══ */}
      <div className="bg-white rounded-xl border border-border-light p-5">
        <h3 className="text-[13px] font-bold text-text mb-3 flex items-center gap-2"><Users size={14} className="text-primary" /> Sign-off & Distribution</h3>
        <div className="grid grid-cols-3 gap-4 text-[12px]">
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Prepared By</span><span className="text-text font-semibold">{finalReport.preparedBy || engagement.owner}</span></div>
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Reviewed By</span><span className="text-text font-semibold">{finalReport.reviewedBy || engagement.reviewer || '—'}</span></div>
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Report Date</span><span className="text-text font-semibold">{finalReport.reportDate || '—'}</span></div>
          <div><span className="text-text-muted block text-[10px] font-medium mb-0.5">Status</span><span className={`px-2 py-0.5 rounded-full text-[9px] font-bold ${REPORT_STATUS_CLS[finalReport.status]}`}>{finalReport.status.replace(/_/g, ' ')}</span></div>
          <div className="col-span-2"><span className="text-text-muted block text-[10px] font-medium mb-0.5">Distribution</span><span className="text-text font-semibold">{finalReport.distributionList || '—'}</span></div>
        </div>
        {isIssued && finalReport.issuedAt && (
          <div className="mt-3 flex items-start gap-2 px-3 py-2 rounded-lg bg-emerald-50 border border-emerald-200 text-[11px] text-emerald-700">
      ...
      {/* ══ Appendix ══ */}
      <div className="bg-white rounded-xl border border-border-light p-5">
        <h3 className="text-[13px] font-bold text-text mb-2 flex items-center gap-2"><FileText size={14} className="text-primary" /> Appendix</h3>
      ...
      <div className="flex items-center gap-3 flex-wrap">
        <button onClick={() => alert('Report download will be connected later.')}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-border-light text-[11px] font-medium text-text-muted hover:bg-surface-2/30 cursor-pointer transition-colors">
          <Download size={12} />Download Draft
        </button>
        {isDraft && (
          <button onClick={handleMarkReady}
            className="px-3 py-1.5 rounded-lg bg-purple-600 hover:bg-purple-700 text-white text-[11px] font-semibold cursor-pointer transition-colors">Mark Ready for Review</button>
        )}
        {isReadyForReview && (
