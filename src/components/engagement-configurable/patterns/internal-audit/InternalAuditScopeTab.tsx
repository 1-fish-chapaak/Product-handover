...
const inputCls = 'w-full px-3 py-2.5 border border-border rounded-lg text-[12.5px] text-text bg-white outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/10 transition-all';
const selectCls = inputCls + ' cursor-pointer appearance-none';
const labelCls = 'text-[11.5px] font-semibold text-text-muted block mb-1.5';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-[17px] font-bold text-text tracking-tight">Scope</h3>
        ...
        <span className={`px-3 py-1.5 rounded-full text-[10px] font-bold shrink-0 ${READINESS_CLS[status]}`}>{status}</span>
      </div>

      {/* ═══ A. Scope Context — compact summary bar ═══ */}
      <div className="rounded-xl border border-border-light bg-surface-2/30 p-4">
        <h4 className="text-[10px] font-bold text-text-muted uppercase tracking-wider mb-3">Scope Context</h4>
        <div className="grid grid-cols-4 gap-4 text-[12px]">
          <div>
            <span className="text-[10px] text-gray-400 block mb-0.5">Assignment</span>
            <span className="text-text font-semibold">{engagement.name}</span>
          </div>
          <div>
            <span className="text-[10px] text-gray-400 block mb-0.5">Scope Level</span>
            <span className="text-text font-semibold">{SCOPE_LEVEL_LABELS[scope.scopeLevel]?.label || scope.scopeLevel}</span>
          </div>
          <div>
            <span className="text-[10px] text-gray-400 block mb-0.5">Entity</span>
            <span className="text-text font-semibold">{engagement.entityOrLocation || '—'}</span>
          </div>
          <div>
            <span className="text-[10px] text-gray-400 block mb-0.5">Process Owner</span>
      ...
      <div className="grid grid-cols-12 gap-6">
        ...
        <div className="col-span-8 space-y-6">
          ...
          <div className="rounded-xl border border-border-light bg-white p-6 space-y-6">
            <div>
              <h4 className="text-[13px] font-bold text-text">Define Scope</h4>
              <p className="text-[11.5px] text-text-muted mt-0.5">Choose the audit coverage level and primary business process.</p>
            </div>

            {/* Scope Level */}
            <div className="space-y-3">
              <label className="text-[11.5px] font-semibold text-text block">Scope Level</label>
              <div className="flex flex-wrap gap-2">
                {Object.entries(SCOPE_LEVEL_LABELS).map(([key, { label }]) => (
                  <button key={key} onClick={() => update('scopeLevel', key)}
                    className={`px-4 py-2 rounded-lg text-[11px] font-semibold cursor-pointer transition-all border ${scope.scopeLevel === key ? 'border-primary bg-primary/8 text-primary shadow-sm shadow-primary/10' : 'border-border-light text-gray-500 hover:border-gray-300 hover:bg-gray-50'}`}>
                    {label}
                  </button>
                ))}
              </div>
              <p className="text-[11px] text-gray-400 leading-relaxed">{SCOPE_LEVEL_LABELS[scope.scopeLevel]?.desc || ''}</p>
            </div>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  {scope.businessProcessId && (
                    <button onClick={() => { setShowAddSubProc(true); setSubProcValidation(''); }}
                      className="flex items-center gap-1 px-2.5 py-1 rounded-lg text-[10px] font-semibold text-primary hover:bg-primary/8 cursor-pointer transition-colors">
                ...
                <div className="flex flex-wrap gap-2">
                  {allSubProcesses.map(sp => (
                    <button key={sp.id} onClick={() => toggleMulti('subProcessIds', sp.id)}
                      className={`px-3 py-1.5 rounded-lg text-[11px] font-medium cursor-pointer transition-all flex items-center gap-1.5 border ${scope.subProcessIds.includes(sp.id) ? 'bg-primary/8 text-primary border-primary/25 shadow-sm shadow-primary/5' : 'bg-gray-50 text-gray-500 border-transparent hover:bg-gray-100 hover:border-gray-200'}`}>
                      {sp.name}
                      {sp.isCustom && <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-purple-50 text-purple-600">Custom</span>}
                ...
                {showAddSubProc && (
                  <div className="mt-2 flex items-center gap-2">
                    <input value={newSubProcName} onChange={e => { setNewSubProcName(e.target.value); setSubProcValidation(''); }} placeholder="Sub-process name..." className="flex-1 px-3 py-2 border border-border rounded-lg text-[11.5px] text-text bg-white outline-none focus:border-primary/40" onKeyDown={e => { if (e.key === 'Enter') addCustomSubProcess(); }} />
                    <button onClick={addCustomSubProcess} disabled={!newSubProcName.trim()} className="px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-white text-[11px] font-semibold cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Add</button>
                    <button onClick={() => { setShowAddSubProc(false); setNewSubProcName(''); setSubProcValidation(''); }} className="px-3 py-2 rounded-lg text-[11px] text-gray-500 hover:text-text hover:bg-gray-100 cursor-pointer transition-colors">Cancel</button>
                  </div>
                )}
                {subProcValidation && <p className="text-[10px] text-red-500 mt-1">{subProcValidation}</p>}
              </div>
            )}
            ...
            {showEmptySubProcesses && (
              <div className="space-y-2">
                ...
                <p className="text-[11px] text-gray-400 italic">No predefined sub-processes for this business process. Add a custom one.</p>
                {showAddSubProc && (
                  <div className="mt-2 flex items-center gap-2">
                    <input value={newSubProcName} onChange={e => { setNewSubProcName(e.target.value); setSubProcValidation(''); }} placeholder="Sub-process name..." className="flex-1 px-3 py-2 border border-border rounded-lg text-[11.5px] text-text bg-white outline-none focus:border-primary/40" onKeyDown={e => { if (e.key === 'Enter') addCustomSubProcess(); }} />
                    <button onClick={addCustomSubProcess} disabled={!newSubProcName.trim()} className="px-4 py-2 rounded-lg bg-primary hover:bg-primary/90 text-white text-[11px] font-semibold cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed">Add</button>
                ...
                <div className="flex flex-wrap gap-2">
                  {availableActivities.map(a => (
                    <button key={a.id} onClick={() => toggleMulti('activityIds', a.id)}
                      className={`px-3 py-1.5 rounded-lg text-[11px] font-medium cursor-pointer transition-all border ${scope.activityIds.includes(a.id) ? 'bg-primary/8 text-primary border-primary/25 shadow-sm shadow-primary/5' : 'bg-gray-50 text-gray-500 border-transparent hover:bg-gray-100 hover:border-gray-200'}`}>
                      {a.name}
                    </button>
                  ))}
                </div>
          ...
          <div className="rounded-xl border border-border-light bg-white p-6 space-y-5">
            <div>
              <h4 className="text-[13px] font-bold text-text">Attach Scope Sources</h4>
              <p className="text-[11.5px] text-text-muted mt-0.5">Attach SOPs, checklists, or RACMs to define what will be reviewed.</p>
            </div>
            ...
            <div className="rounded-lg border border-border-light/70 bg-gray-50/40 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-[12px] font-semibold text-text block">SOPs</span>
                    <span className="text-[10px] text-gray-400">Recommended</span>
                  </div>
                </div>
                <button onClick={() => alert('Upload SOP will be connected later.')}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[10px] font-semibold text-primary bg-white hover:bg-primary/5 cursor-pointer transition-colors border border-border-light">
              ...
              <div className="flex flex-wrap gap-2">
                {SOPS.map(s => (
                  <button key={s.id} onClick={() => toggleMulti('sopIds', s.id)}
                    className={`group px-3.5 py-2 rounded-lg text-[11px] font-medium cursor-pointer transition-all flex items-center gap-2 border ${scope.sopIds.includes(s.id) ? 'bg-primary/8 text-primary border-primary/25 shadow-sm' : 'bg-white text-gray-600 border-border-light hover:border-gray-300 hover:shadow-sm'}`}>
                    <FileText size={11} className={scope.sopIds.includes(s.id) ? 'text-primary' : 'text-gray-400'} />
                    <span>{s.name}</span>
                    <span className={`text-[9px] ${scope.sopIds.includes(s.id) ? 'text-primary/60' : 'text-gray-400'}`}>{s.version}</span>
            ...
            <div className="rounded-lg border border-border-light/70 bg-gray-50/40 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-[12px] font-semibold text-text block">Checklists</span>
              ...
              <div className="flex flex-wrap gap-2">
                {CHECKLISTS.map(c => (
                  <button key={c.id} onClick={() => toggleMulti('checklistIds', c.id)}
                    className={`group px-3.5 py-2 rounded-lg text-[11px] font-medium cursor-pointer transition-all flex items-center gap-2 border ${scope.checklistIds.includes(c.id) ? 'bg-primary/8 text-primary border-primary/25 shadow-sm' : 'bg-white text-gray-600 border-border-light hover:border-gray-300 hover:shadow-sm'}`}>
                    <ClipboardCheck size={11} className={scope.checklistIds.includes(c.id) ? 'text-primary' : 'text-gray-400'} />
                    <span>{c.name}</span>
                    <span className={`text-[9px] ${scope.checklistIds.includes(c.id) ? 'text-primary/60' : 'text-gray-400'}`}>· {c.items} items</span>
            ...
            <div className="rounded-lg border border-border-light/70 bg-gray-50/40 p-4 space-y-3">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <span className="text-[12px] font-semibold text-text block">RACM</span>
                    <span className="text-[10px] text-gray-400">Optional</span>
                  </div>
                </div>
              ...
              <div className="flex flex-wrap gap-2">
                {RACMS.map(r => (
                  <button key={r.id} onClick={() => toggleMulti('racmVersionIds', r.id)}
                    className={`group px-3.5 py-2 rounded-lg text-[11px] font-medium cursor-pointer transition-all flex items-center gap-2 border ${scope.racmVersionIds.includes(r.id) ? 'bg-primary/8 text-primary border-primary/25 shadow-sm' : 'bg-white text-gray-600 border-border-light hover:border-gray-300 hover:shadow-sm'}`}>
                    <Shield size={11} className={scope.racmVersionIds.includes(r.id) ? 'text-primary' : 'text-gray-400'} />
                    <span>{r.name}</span>
                    <span className={`text-[9px] ${scope.racmVersionIds.includes(r.id) ? 'text-primary/60' : 'text-gray-400'}`}>{r.version}</span>
          ...
          <div className="rounded-xl border border-border-light bg-white p-6 space-y-5">
            <div>
              <h4 className="text-[13px] font-bold text-text">Scope Narrative</h4>
              <p className="text-[11.5px] text-text-muted mt-0.5">Describe the audit objective, in-scope, and out-of-scope areas.</p>
            </div>
            <div>
        ...
        <div className="col-span-4 space-y-6">
          ...
          <div className="rounded-xl border border-border-light bg-white p-5 space-y-4 sticky top-4">
            <h4 className="text-[12px] font-bold text-text">Scope Summary</h4>
            ...
            <div className="space-y-2.5 text-[11px]">
              <div className="flex items-center gap-2">
                <span className="text-gray-400 w-[72px] shrink-0 text-[10px]">Level</span>
                <span className="text-text font-medium">{SCOPE_LEVEL_LABELS[scope.scopeLevel]?.label || '—'}</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-gray-400 w-[72px] shrink-0 text-[10px]">Process</span>
                <span className="text-text font-medium">{selectedBP?.name || '—'}</span>
              </div>
              {scope.subProcessIds.length > 0 && (
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 w-[72px] shrink-0 text-[10px] pt-0.5">Sub-proc</span>
                  <div className="flex flex-wrap gap-1.5">
                    {selectedSubProcesses.map(sp => (
                      <span key={sp.id} className="px-2 py-0.5 rounded bg-primary/10 text-primary text-[10px] font-semibold">
              ...
              {scope.sopIds.length > 0 && (
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 w-[72px] shrink-0 text-[10px] pt-0.5">SOPs</span>
                  <div className="flex flex-wrap gap-1.5">
                    {scope.sopIds.map(id => { const s = SOPS.find(x => x.id === id); return s ? <span key={id} className="px-2 py-0.5 rounded bg-primary/10 text-primary text-[10px] font-semibold">{s.name}</span> : null; })}
                  </div>
                </div>
              )}
              {scope.checklistIds.length > 0 && (
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 w-[72px] shrink-0 text-[10px] pt-0.5">Checklists</span>
                  <div className="flex flex-wrap gap-1.5">
                    {scope.checklistIds.map(id => { const c = CHECKLISTS.find(x => x.id === id); return c ? <span key={id} className="px-2 py-0.5 rounded bg-primary/10 text-primary text-[10px] font-semibold">{c.name}</span> : null; })}
                  </div>
                </div>
              )}
              {scope.racmVersionIds.length > 0 && (
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 w-[72px] shrink-0 text-[10px] pt-0.5">RACM</span>
                  <div className="flex flex-wrap gap-1.5">
                    {scope.racmVersionIds.map(id => { const r = RACMS.find(x => x.id === id); return r ? <span key={id} className="px-2 py-0.5 rounded bg-primary/10 text-primary text-[10px] font-semibold">{r.name}</span> : null; })}
                  </div>
                </div>
              )}
              {scope.inScopeItems && (
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 w-[72px] shrink-0 text-[10px] pt-0.5">In Scope</span>
                  <span className="text-text text-[11px]">{scope.inScopeItems}</span>
                </div>
              )}
              {scope.outOfScopeItems && (
                <div className="flex items-start gap-2">
                  <span className="text-gray-400 w-[72px] shrink-0 text-[10px] pt-0.5">Out Scope</span>
                  <span className="text-text text-[11px]">{scope.outOfScopeItems}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border-light bg-white p-5 space-y-4">
            <div>
              <div className="flex items-center justify-between mb-1">
                <h4 className="text-[12px] font-bold text-text">Scope Readiness</h4>
                <span className={`px-2.5 py-1 rounded-full text-[9px] font-bold ${READINESS_CLS[status]}`}>{status}</span>
              </div>
              <p className="text-[10.5px] text-text-muted leading-relaxed">Complete the required scope details before sending the audit announcement.</p>
            </div>
            ...
            <div className="space-y-2.5">
              {checks.map(c => (
                <div key={c.label} className="flex items-center gap-2.5 text-[11px]">
                  ...
                  {!c.required && <span className="text-[9px] text-gray-300 shrink-0">Optional</span>}
                </div>
              ))}
            </div>
            ...
            <div className="pt-2 border-t border-border-light/60">
              <button onClick={() => onNavigateTab?.('announcement')} disabled={status === 'Draft Scope'}
                className="w-full flex items-center justify-center gap-1.5 px-4 py-2.5 rounded-lg bg-primary hover:bg-primary/90 text-white text-[12px] font-semibold cursor-pointer transition-colors disabled:opacity-40 disabled:cursor-not-allowed shadow-sm shadow-primary/20">
