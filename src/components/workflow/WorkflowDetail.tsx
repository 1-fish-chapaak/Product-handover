    ...
    <div className="flex flex-wrap items-center gap-2">
      {items.map((it, idx) =>
        editingIdx === idx ? (
          <input
            ...
            className="px-2.5 py-1 rounded-full text-[0.75rem] border border-primary/50 outline-none ring-2 ring-primary/15 bg-white text-text w-36"
          />
        ) : (
          <span
            key={idx}
            className="inline-flex items-center gap-1.5 pl-3 pr-1.5 py-1 rounded-full bg-surface-2 border border-border-light text-[0.75rem] font-medium text-text"
          >
            {it}
            <button
      ...
      <div className="inline-flex items-center gap-1">
        <input
          ...
          className="px-3 py-1.5 rounded-full text-[0.75rem] border border-border-light bg-white text-text placeholder:text-text-muted outline-none focus:border-primary/40 focus:ring-2 focus:ring-primary/10 w-40"
        />
        <button
          onClick={add}
          disabled={!draft.trim()}
      ...
      <div className="rounded-2xl border border-border-light bg-gradient-to-br from-primary-xlight/40 to-white p-7 mb-6 relative overflow-hidden shrink-0">
        ...
        <div className="flex items-center justify-between gap-6 mb-5">
          <div className="flex items-center gap-3 shrink-0">
            ...
            <span className="inline-flex items-center gap-1.5 text-[0.6875rem] font-medium text-ink-500">
        ...
        {/* Title */}
        <h1 className="text-[1.75rem] font-semibold tracking-tight text-ink-900 leading-[1.15]">
          {wf.name}
        </h1>

        {/* Bottom row: schedule meta (left) + primary CTA (right) */}
        <div className="flex items-center justify-between gap-6 mt-3">
          <div className="flex items-center gap-3 text-[0.75rem] text-text-muted min-w-0">
        ...
        {effStatus === 'Error' && effError && (
          <div className="mt-4 flex items-center gap-3 rounded-lg border border-risk-100 bg-risk-50 px-3.5 py-2.5 text-[0.75rem] text-risk-700">
            ...
            <button
              ...
              className="shrink-0 inline-flex items-center gap-1.5 px-3 h-8 rounded-md border border-risk-200 bg-white text-[0.75rem] font-semibold text-risk-700 hover:bg-risk-50 disabled:opacity-60 disabled:cursor-default cursor-pointer transition-colors"
            >
      ...
      {tab === 'overview' && (
        <div className="space-y-5 pb-8">
          ...
          {wf.linkedControls.length > 0 && (
            <div>
              <h4 className="text-[0.875rem] font-semibold text-ink-900 mb-3">
                Linked controls <span className="text-text-muted font-normal">({wf.linkedControls.length})</span>
              </h4>
              <div className="space-y-2">
                {wf.linkedControls.map(c => (
                  <div key={c.id} className="rounded-lg border border-border-light bg-white px-4 py-3 flex items-center gap-3">
                    <span className="font-mono text-[0.75rem] font-semibold text-brand-700 shrink-0">{c.id}</span>
                    <span className="text-[0.8125rem] text-text min-w-0 truncate">{c.name}</span>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="space-y-5 pb-8">
          <div className="rounded-lg border border-border-light bg-white p-5">
            ...
            <div className="space-y-5">
              <div>
                <label className="text-[0.8125rem] font-semibold text-text block mb-1">Labels</label>
                <p className="text-[0.75rem] text-text-muted mb-2.5">Tag this workflow for grouping and search — e.g. Reconciliation.</p>
                <EditableChipList
                  items={labels}
                  onChange={setLabels}
              ...
              <div className="pt-4 border-t border-border-light">
                <label className="text-[0.8125rem] font-semibold text-text block mb-1">Sub-process</label>
                <p className="text-[0.75rem] text-text-muted mb-2.5">Add, rename, or remove the sub-processes this workflow belongs to.</p>
                <EditableChipList
                  items={subProcesses}
