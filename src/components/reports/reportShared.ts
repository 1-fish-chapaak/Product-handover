...
/** The letterhead line every template prints unless a read of the client's own
 *  report replaced it. There is no field for it in the editor: a template that
 *  came from their file carries their stamp, and one built by hand carries this
 *  one, so the letterhead is never blank. */
export const DEFAULT_HEADER_TEXT = 'Confidential — For Internal Use Only';
