...
/** The two facts a template's cover carries, in one place, because the same
 *  cover is drawn on three surfaces (the editor's live page, the check screen's
 *  to-be page and the template sheet) and they must not drift apart. Neither is
 *  a template setting: the organisation is typed once and the period comes from
 *  whichever report is generated. */
export const templateCoverFields = (brand?: string): { label: string; value: string }[] => [
  ...
  { label: 'Period', value: 'Set per report' },
];
