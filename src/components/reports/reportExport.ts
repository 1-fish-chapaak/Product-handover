...
const MUTED = '#6B5D82';
const BRAND = '#6A12CD';
const BRAND_DARK = '#550FA5';   // brand-700 — banner background
...
// Brand banner — IRAME.AI lockup, report title, byline. Single-cell table so
// Word honours the background fill.
function brandBanner(ctx: ReportExportContext): string {
  ...
  return `<table width="100%" cellpadding="0" cellspacing="0"><tr>
    <td bgcolor="${BRAND_DARK}" style="background:${BRAND_DARK};padding:26px 36px;">
      <p style="margin:0 0 16px;line-height:1.2;">
        <span style="color:#FFFFFF;font-weight:bold;font-size:11pt;letter-spacing:1px;">IRAME.AI</span><br/>
        <span style="color:#D8C2F2;font-size:6.5pt;font-weight:bold;letter-spacing:3px;">AUDIT INTELLIGENCE</span>
      </p>
      <h1 style="color:#FFFFFF;font-size:21pt;margin:0;line-height:1.15;">${esc(ctx.reportName)}</h1>
      ${byline ? `<p style="color:#E3D3F7;font-size:10pt;margin:6px 0 0;">${byline}</p>` : ''}
    </td>
  </tr></table>`;
}
...
function numberedHeading(n: number, title: string, sub?: string): string {
  return `<p style="margin:22px 0 ${sub ? '2px' : '10px'};">
    ...
    <span style="font-size:13pt;font-weight:bold;color:${INK};">&nbsp; ${esc(title)}</span>
...
function slideShell(inner: string, background?: string): string {
  return `<div style="page-break-after:always;width:960px;height:540px;box-sizing:border-box;padding:48px 56px;font-family:Calibri,Arial,sans-serif;color:${INK};border-bottom:1px solid #eee;${background ? `background:${background};` : ''}">${inner}</div>`;
}

export function exportReportPpt(ctx: ReportExportContext) {
  const slides: string[] = [];
  // Title slide — brand banner treatment, matching the on-screen report cover.
  slides.push(slideShell(`
    <p style="margin:0 0 110px;line-height:1.2;">
      <span style="color:#FFFFFF;font-weight:bold;font-size:14pt;letter-spacing:1.5px;">IRAME.AI</span><br/>
      <span style="color:#D8C2F2;font-size:8pt;font-weight:bold;letter-spacing:4px;">AUDIT INTELLIGENCE</span>
    </p>
    <h1 style="font-size:30pt;margin:0 0 12px;color:#FFFFFF;">${esc(ctx.reportName)}</h1>
    <p style="font-size:13pt;color:#E3D3F7;margin:0;">${[ctx.reportTag, `Generated by ${ctx.generatedBy}`, ctx.generatedAt].filter(Boolean).map(esc).join(' · ')}</p>`, BRAND_DARK));
  ...
  for (const s of ctx.sections) {
    if (s.kind === 'summary') {
      ...
      slides.push(slideShell(`
        <h2 style="font-size:22pt;color:${BRAND};margin:0 0 16px;">${esc(s.title)}</h2>
      ...
      slides.push(slideShell(`
        ...
        <h2 style="font-size:18pt;color:${INK};margin:0 0 14px;">${esc(s.queryTitle)}</h2>
      ...
      slides.push(slideShell(`
        ...
        <h2 style="font-size:18pt;margin:0 0 14px;">${esc(s.workflowName)}</h2>
        ${bullets ? `<ul style="font-size:12.5pt;line-height:1.5;margin:0;">${bullets}</ul>` : `<p style="font-size:12.5pt;">${esc(plain(s.summary))}</p>`}`));
    } else if (s.kind === 'observation') {
      slides.push(slideShell(`
        <p style="font-size:10pt;color:${MUTED};margin:0 0 4px;">${esc(s.obsId)}</p>
        <h2 style="font-size:18pt;margin:0 0 14px;">${esc(s.title)}</h2>
