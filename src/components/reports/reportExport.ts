...
function brandBanner(ctx: ReportExportContext): string {
  ...
  const org = ctx.orgBrand?.trim();
  return `<table width="100%" cellpadding="0" cellspacing="0">
    <tr><td bgcolor="${p.from}" style="background:${p.from};background:linear-gradient(135deg, ${p.from}, ${p.to});padding:34px 36px;">
      ${org ? `<p style="color:${p.byline};font-size:8.5pt;font-weight:bold;letter-spacing:1.4px;text-transform:uppercase;margin:0 0 8px;">${esc(org)}</p>` : ''}
...
export function exportReportPpt(ctx: ReportExportContext) {
  ...
  slides.push(slideShell(`
    ${ctx.orgBrand?.trim() ? `<p style="font-size:11pt;font-weight:bold;letter-spacing:1.6px;text-transform:uppercase;color:${p.byline};margin:150px 0 10px;">${esc(ctx.orgBrand.trim())}</p><h1 style="font-family:${TITLE_FONT};font-weight:bold;font-size:32pt;margin:0 0 14px;color:#FFFFFF;">${esc(ctx.reportName)}</h1>` : `<h1 style="font-family:${TITLE_FONT};font-weight:bold;font-size:32pt;margin:150px 0 14px;color:#FFFFFF;">${esc(ctx.reportName)}</h1>`}
