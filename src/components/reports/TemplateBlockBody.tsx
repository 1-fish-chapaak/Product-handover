...
function BlockBody({ block, cards, findingScale, composed, manual, facts }: {
  block: TemplateBlock;
  cards: CardFinding[];
  findingScale?: string[];
  ...
  if (block.kind === 'cards') {
    ...
    if (cards.length === 0) {
      return (
        <BoundEmpty
          message={bound.kind === 'rows' && bound.empty
            ? bound.emptyMessage
  ...
  if (block.kind === 'table') {
    ...
    const linkedRows = block.linkedTo || block.binding === 'actions' || block.severity
      ? boundRows
      : (block.linkedTo && cards.length > 0 ? cards : []);
    if ((block.linkedTo || block.binding === 'actions') && bound.kind === 'rows' && bound.empty) {
      return (
        <BoundEmpty
          message={bound.emptyMessage}
          shape={cols.length ? `Columns kept from your report: ${cols.join(', ')}.` : undefined}
        />
      );
    }
    // A table whose columns map to nothing we hold would print a grid of
    // dashes, which reads as broken rather than as empty. Say it instead.
    const mappedCols = cols.filter(c => fieldValue(c, { title: '', severity: '' }, '', '') !== null).length;
    if (linkedRows.length > 0 && cols.length > 0 && mappedCols === 0) {
      return (
        <BoundEmpty
          message="This table keeps your columns, but they are not ones we hold data for, so it prints empty for you to fill."
...
export default function TemplateBlockBody({ tsec, cards = [], findingScale, composed, manual, blockLibrary, facts }: {
    ...
    <div className="space-y-5">
      {rendered.map((b, i) => (
        <div key={i}>
          ...
          <BlockBody block={b} cards={cards} findingScale={findingScale} composed={composed} manual={i === manualIdx ? manual : undefined} facts={facts} />
        </div>
      ))}
    </div>
  );
