              ...
              <div key={o._id} className="grid grid-cols-[1fr_1.3fr_88px_120px] gap-3 px-3 py-2.5 items-start bg-canvas-elevated">
                ...
                <div className="min-w-0 relative">
                  <div className="flex flex-wrap gap-1">
                    {o.annexures.map(a => (
                      <span key={a.id} className="inline-flex items-center gap-1 h-6 pl-1.5 pr-1 rounded-[6px] bg-paper-50 border border-canvas-border text-[0.625rem] text-ink-700 max-w-full">
                    ...
                    <button onClick={() => setPicker(picker === o._id ? null : o._id)} className="inline-flex items-center gap-1 h-6 px-2 rounded-[6px] border border-dashed border-canvas-border text-[0.625rem] font-semibold text-brand-700 hover:border-brand-300 cursor-pointer">
                  ...
                  {picker === o._id && (
                    <>
                      ...
                      <div className="absolute left-0 top-full mt-1 z-[70] w-72 max-h-56 overflow-y-auto bg-white border border-canvas-border shadow-xl rounded-[10px] p-1">
                        {available.length === 0 ? (
                          <div className="px-3 py-3 text-[0.6875rem] text-ink-500 text-center">All annexures linked.</div>
                        ) : available.map(a => (
                          <button key={a.id} onClick={() => addAnnexure(o._id, a.id)} title={a.name} className="w-full flex items-center gap-2 px-2.5 py-2 rounded-[7px] hover:bg-brand-50 text-left cursor-pointer">
