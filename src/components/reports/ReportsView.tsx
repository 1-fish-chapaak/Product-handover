...
export default function ReportsView({
  ...
  const [customTemplatesLocal, setCustomTemplatesLocal] = useState<EditableTemplate[]>(() => {
    try {
      ...
      if (raw) {
        ...
        // Same one-format-at-a-time rule the app-level store applies, so this
        // fallback cannot show two formats both claiming to be the one internal
        // audit reports come out in.
        if (Array.isArray(parsed)) return oneDefaultOnly(parsed as EditableTemplate[]);
      }
  ...
  // Another tab changed the reports — applied a template, renamed, deleted. The
  // storage event fires only in the OTHER tabs, so this cannot loop with the
  // write above. The report open here is refreshed too, or it would keep
  // showing a template the other tab has already moved off.
  useEffect(() => {
    const onStorage = (e: StorageEvent) => {
      ...
      try {
        ...
        const next = (parsed as GeneratedReport[]).filter(r => !(r as { isEmpty?: boolean }).isEmpty);
        setGeneratedReports(next);
        setViewingReport(prev => (prev ? next.find(r => r.id === prev.id) ?? prev : prev));
  ...
  const reportPills = (r: GeneratedReport) => {
    ...
    // Both counts come off the shared helpers rather than the seed record, so
    // the card promises exactly what the reader opens with.
    // A bulk audit renders workflow runs, not queries, so it counts what it
    ...
    const body = w > 0
      ...
      : `${q} ${q === 1 ? 'query' : 'queries'}`;
    return [body, `${sec} ${sec === 1 ? 'section' : 'sections'}`];
  };
  ...
  // The query set a shared record materialises into. The advertised `queries`
  // number can exceed the pool, so the pool is the ceiling and the row chip
  // reads off this same function — the card cannot promise more than opens.
  const sharedReportQueries = (r: typeof SHARED_REPORTS[number]): GeneratedQueryDef[] => {
  ...
  const openSharedReport = (r: typeof SHARED_REPORTS[number]) => {
    const generatedQueries = sharedReportQueries(r);
    setViewingReport({
      ...r,
    ...
    return (
      <TemplatePreview
        ...
        onDuplicate={() => {
          ...
          // opens in the editor so the duplicate is immediately usable.
          const copy: EditableTemplate = {
        ...
        {activeTab === 'my-reports' && reportType === 'all' && (
          ...
          <SmartTable
            ...
            columns={[
              ...
              { key: 'source', label: 'Format', width: COL_W.source, render: (item) => (
            ...
            columns={[
              { key: 'name', label: 'Report', render: (item) => (
                <ReportNameCell
                  ...
                  name={String(item.name)}
                  subline={(() => { const n = sharedReportQueries(item as unknown as typeof SHARED_REPORTS[number]).length; return `${n} ${n === 1 ? 'query' : 'queries'}`; })()}
              ...
              { key: 'sharedAt', label: 'Shared', width: COL_W.generated, render: (item) => (
                ...
                <div className="flex items-center justify-end gap-1.5 opacity-60 group-hover:opacity-100 group-focus-within:opacity-100 transition-opacity">
                  ...
                  {can('rp_share') && <ActionTooltip label="Share"><button onClick={(e) => { e.stopPropagation(); openShare({ type: 'report', id: String(item.id), name: String(item.name), anchor: rectFromEvent(e) }); }} className="inline-flex items-center justify-center w-7 h-7 rounded-md border border-canvas-border bg-canvas-elevated text-ink-500 hover:border-ink-300/70 hover:text-brand-700 hover:bg-canvas transition-colors cursor-pointer" aria-label="Share"><Share2 size={14} /></button></ActionTooltip>}
                </div>
              )},
            ]}
          />
            ...
            <InfiniteCardGrid
              ...
              renderItem={(r, i) => {
                ...
                <ReportCard
                  ...
                  pills={[(() => { const n = sharedReportQueries(r).length; return `${n} ${n === 1 ? 'query' : 'queries'}`; })(), r.sharedWith]}
                  ...
                  actions={<>
                    ...
                    {can('rp_share') && <ActionTooltip label="Share"><button onClick={(e) => { e.stopPropagation(); openShare({ type: 'report', id: r.id, name: r.name, anchor: rectFromEvent(e) }); }} className="inline-flex items-center justify-center w-7 h-7 rounded-md border border-canvas-border bg-canvas-elevated text-ink-500 hover:border-ink-300/70 hover:text-brand-700 hover:bg-canvas transition-colors cursor-pointer" aria-label="Share"><Share2 size={14} /></button></ActionTooltip>}
                  </>}
                />
                );
              }}
