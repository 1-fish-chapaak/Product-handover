      ...
      <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                <input
                  ...
                  className="text-[15px] font-semibold text-ink-800 bg-transparent border-none outline-none focus:underline focus:decoration-brand-600/30 block w-full"
                />
                <p className="text-[11px] text-ink-400">Template Configuration</p>
              </div>
            </div>
          </div>
          <button
        ...
        {showPreview && (
          <div className="flex-1 overflow-y-auto py-5 px-6">
            <div className="bg-white rounded-[12px] border border-canvas-border overflow-hidden shadow-sm">
              {/* Report Header */}
              <div className="px-8 py-4 flex items-center justify-between" style={{ background: `${templateThemeColor}15` }}>
                <div>
                  ...
                  <div className="text-[18px] font-bold text-ink-800">{title}</div>
                </div>
                <div className="text-right">
                  <div className="text-[10px] text-ink-400">{templateHeader}</div>
                  <div className="text-[10px] text-ink-400 mt-0.5">{new Date().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</div>
                </div>
              </div>
              ...
              <div className="px-8 py-5 flex flex-col gap-5">
                {sections.length === 0 ? (
                  <div className="text-center py-10 text-[12px] text-ink-400">No sections added yet. Add report sections to see them here.</div>
                ) : sections.map(section => (
                  <div key={section.id}>
                    <div className="flex items-center gap-2 mb-2">
                      <div className="h-3 w-1 rounded-full" style={{ background: templateThemeColor }} />
                      <span className="text-[12px] font-bold text-ink-800">{section.title}</span>
                    </div>
                    {section.type === 'text' && (
              ...
              <div className="px-8 py-3 border-t border-canvas-border flex items-center justify-between" style={{ background: `${templateThemeColor}08` }}>
                <span className="text-[9px] text-ink-400">{templateFooter}</span>
                <span className="text-[9px] text-ink-400">Page 1 of 1</span>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="mb-2">
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  <label htmlFor="template-name-input" className="text-[12px] font-semibold text-ink-800 block mb-1.5">
                ...
                <div>
                  <label className="text-[12px] font-semibold text-ink-800 block mb-1.5">Category</label>
                  <div className="relative">
                    <button
                      onClick={() => setShowCategoryDropdown(p => !p)}
                      className="w-full flex items-center justify-between px-3 py-2 rounded-[8px] border border-canvas-border text-[12px] text-ink-800 hover:border-brand-600/30 transition-colors cursor-pointer"
                    >
                      {templateCategory}
                ...
                <div>
                  <label className="text-[12px] font-semibold text-ink-800 block mb-1.5">Header Text</label>
                  <input value={templateHeader} onChange={e => setTemplateHeader(e.target.value)} className="w-full px-3 py-2 rounded-[8px] border border-canvas-border text-[12px] focus:outline-none focus:border-brand-600/40 focus:ring-2 focus:ring-brand-600/10" />
                </div>
                <div>
                  <label className="text-[12px] font-semibold text-ink-800 block mb-1.5">Footer Text</label>
                  <input value={templateFooter} onChange={e => setTemplateFooter(e.target.value)} className="w-full px-3 py-2 rounded-[8px] border border-canvas-border text-[12px] focus:outline-none focus:border-brand-600/40 focus:ring-2 focus:ring-brand-600/10" />
                </div>
              </div>
              <div className="mb-3">
                <label className="text-[12px] font-semibold text-ink-800 block mb-1.5">Theme Color</label>
              ...
              <div className="border border-canvas-border rounded-[12px] p-3 bg-canvas">
                <div className="bg-white rounded-[12px] shadow-sm border border-canvas-border overflow-hidden" style={{ aspectRatio: '8.5/3' }}>
                  <div className="h-5 flex items-center justify-between px-3" style={{ background: `${templateThemeColor}10` }}>
                    ...
                    <span className="text-[5px] text-ink-400">{templateHeader}</span>
                  </div>
                  ...
                  <div className="h-3 bg-canvas flex items-center justify-center">
                    <span className="text-[4px] text-ink-400">{templateFooter}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            {/* Add Report Sections */}
            <div>
              <h4 className="text-[12px] font-bold text-ink-400 mb-3 flex items-center gap-2">
                ...
                {AVAILABLE_BLOCKS.filter(b => b.type !== 'action-taken' || context === 'action-report').map(block => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="text-[12px] font-semibold text-ink-800 group-hover:text-brand-600 transition-colors">{block.label}</span>
                    </div>
                    <p className="text-[11px] text-ink-400 pl-7">{block.desc}</p>
                  </button>
                ))}
              </div>
              <button
                ...
                className="mt-3 w-full flex items-center justify-center gap-1.5 px-3 py-2 rounded-[8px] border border-dashed border-canvas-border text-[12px] font-medium text-ink-500 hover:bg-paper-50 hover:border-brand-600/30 transition-colors cursor-pointer disabled:opacity-60 disabled:cursor-wait focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600/40 focus-visible:ring-offset-1"
              >
            ...
            {sections.length > 0 && (
              <h4 className="text-[12px] font-bold text-ink-400 px-1 pt-2">Detected Sections</h4>
            )}
              ...
              {sections.map((section, index) => (
                ...
                >
                  <div className="flex items-center gap-3 px-4 py-3 border-b border-canvas-border">
                    ...
                    <div className="w-6 h-6 rounded-[8px] bg-brand-600/10 text-brand-600 text-[11px] font-bold flex items-center justify-center shrink-0">
                      {index + 1}
                    </div>
                    <input
                      value={section.title}
                      onChange={e => setSections(prev => prev.map(s => s.id === section.id ? { ...s, title: e.target.value } : s))}
                      className="flex-1 text-[13px] font-semibold text-ink-800 bg-transparent border-none outline-none"
                    />
                  ...
                  <div className="px-4 py-3">
                    {section.type === 'text' ? (
                      <div className="flex items-center gap-2 p-3 bg-canvas rounded-[12px]">
                        ...
                        <div>
                          <div className="text-[12px] text-ink-400">Content added from /chat</div>
                          {section.content && <div className="text-[12px] text-ink-500 mt-0.5">{section.content}</div>}
                        </div>
                      </div>
                    ) : section.type === 'exception-summary' ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-[12px] text-ink-400">Total flagged amount: $616,650</div>
                      </div>
                    ) : section.type === 'action-taken' ? (
            ...
            >
              <div className="px-5 py-4 border-b border-canvas-border flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <h4 id="save-template-title" className="text-[14px] font-semibold text-ink-800">Save as Template</h4>
                </div>
                <button
                  onClick={() => setShowSaveTemplate(false)}
              ...
              <div className="p-5 space-y-4">
                ...
                <div>
                  <label htmlFor="modal-template-name" className="text-[12px] font-semibold text-ink-800 mb-1.5 block">
                    Template Name <span className="text-risk ml-0.5 font-normal">*</span>
                  </label>
                  <input
                ...
                <div>
                  <label className="text-[12px] font-semibold text-ink-800 mb-1.5 block">Description</label>
                  <textarea
                    value={templateDesc}
                    onChange={e => setTemplateDesc(e.target.value)}
                    className="w-full px-3 py-2.5 rounded-[8px] border border-canvas-border text-[13px] focus:outline-none focus:border-brand-600/40 focus:ring-2 focus:ring-brand-600/10 resize-none min-h-[70px]"
                    placeholder="Describe what this template is for..."
                  />
                </div>
                <div className="relative">
                  <label className="text-[12px] font-semibold text-ink-800 mb-1.5 block">Category</label>
                  <button
                    onClick={() => setShowCategoryDropdown(p => !p)}
                    className="w-full flex items-center justify-between px-3 py-2.5 rounded-[8px] border border-canvas-border text-[13px] text-ink-800 hover:border-brand-600/30 transition-colors cursor-pointer"
                  >
                    {templateCategory}
                      ...
                      >
                        {TEMPLATE_CATEGORIES.map(cat => (
                          <button
                            ...
                            className={`w-full text-left px-3 py-2 text-[12px] hover:bg-brand-50 transition-colors cursor-pointer ${templateCategory === cat ? 'text-brand-600 font-semibold bg-brand-600/5' : 'text-ink-800'}`}
                          >
                            {cat}
                          </button>
                        ))}
