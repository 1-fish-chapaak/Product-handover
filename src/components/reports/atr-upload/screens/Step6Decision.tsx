    ...
    <div className="w-full">
      ...
      <div className="grid grid-cols-2 gap-4 max-w-[720px] items-stretch">
        ...
        <button
          ...
          className={`relative flex flex-col text-left rounded-[16px] border p-5 transition-all duration-200 cursor-pointer group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand-600/40 ${active === 'generate' ? 'border-brand-500 bg-brand-50/50 shadow-md shadow-brand-900/[0.06]' : 'border-canvas-border bg-canvas-elevated hover:border-brand-300 hover:bg-brand-50/20 hover:-translate-y-0.5 hover:shadow-md hover:shadow-brand-900/[0.05]'}`}
        >
          <span className={`w-12 h-12 rounded-[13px] flex items-center justify-center mb-3 transition-all duration-200 ${active === 'generate' ? 'bg-brand-600 text-white shadow-md shadow-brand-600/30 scale-[1.03]' : 'bg-brand-50 text-brand-700'}`}><FileText size={21} aria-hidden="true" /></span>
        ...
        <button
          ...
          className={`relative flex flex-col text-left rounded-[16px] border p-5 transition-all duration-200 group focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-evidence-600/40 ${!canManage ? 'border-canvas-border bg-canvas-elevated opacity-55 cursor-not-allowed' : active === 'manage' ? 'border-evidence-500 bg-evidence-50/40 shadow-md shadow-brand-900/[0.06] cursor-pointer' : 'border-canvas-border bg-canvas-elevated hover:border-evidence-300 hover:bg-evidence-50/20 hover:-translate-y-0.5 hover:shadow-md hover:shadow-brand-900/[0.05] cursor-pointer'}`}
        >
          <span className={`w-12 h-12 rounded-[13px] flex items-center justify-center mb-3 transition-all duration-200 ${active === 'manage' ? 'bg-evidence-600 text-white shadow-md shadow-evidence-600/30 scale-[1.03]' : 'bg-evidence-50 text-evidence-700'}`}><ListChecks size={21} aria-hidden="true" /></span>
