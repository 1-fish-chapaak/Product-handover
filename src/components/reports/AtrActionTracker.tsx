  ...
  return (
    <>
      ...
      <motion.div
        ...
        className="fixed left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-[1040px] max-w-[95vw] h-[82vh] bg-canvas-elevated rounded-xl shadow-xl border border-canvas-border z-[65] flex flex-col" role="dialog" aria-label="Action item tracker"
      >
        ...
        <div className="shrink-0 px-6 py-3 grid grid-cols-4 gap-3 border-b border-canvas-border">
          {KPIS.map(k => (
            <button key={k.key} onClick={() => setStatusFilter(k.key)} className={`text-left rounded-lg border p-3 transition-colors cursor-pointer ${statusFilter === k.key ? 'border-brand-400 ring-2 ring-brand-600/15' : 'border-canvas-border hover:border-brand-200'}`}>
              <div className="flex items-center gap-2 mb-1"><span className={`w-6 h-6 rounded-sm flex items-center justify-center ${k.tone}`}><k.icon size={13} /></span><span className="text-[1.25rem] font-bold tabular-nums text-ink-900 leading-none">{k.value}</span></div>
        ...
        <div className="shrink-0 px-6 py-2.5 flex items-center gap-3 border-b border-canvas-border">
          <div className="relative flex-1 max-w-[320px]">
            ...
            <input value={q} onChange={e => setQ(e.target.value)} placeholder="Search actions, owners, ATRs…" className="w-full h-9 pl-9 pr-3 rounded-md border border-canvas-border bg-canvas text-[0.8125rem] text-ink-900 focus:outline-none focus:border-brand-600 focus:ring-4 focus:ring-brand-600/15" />
          </div>
          <div className="flex items-center gap-1 ml-auto">
            {(['open', 'all'] as const).map(f => (
              <button key={f} onClick={() => setStatusFilter(f)} className={`h-8 px-3 rounded-sm text-[0.75rem] font-semibold capitalize transition-colors cursor-pointer ${statusFilter === f ? 'bg-brand-600 text-white' : 'bg-paper-50 text-ink-600 hover:bg-paper-100'}`}>{f === 'open' ? 'Open only' : 'All'}</button>
            ))}
          </div>
        </div>
                ...
                {filtered.map(r => {
                  ...
                  return (
                    <tr key={r.id} className="border-t border-canvas-border hover:bg-paper-50/60">
                      ...
                      <td className="px-3 py-2.5"><button onClick={() => { onClose(); onOpen(r.atrId); }} title="Open ATR" className="w-7 h-7 rounded-sm flex items-center justify-center text-ink-400 hover:text-brand-700 hover:bg-brand-50 cursor-pointer"><ExternalLink size={13} /></button></td>
                    </tr>
                  );
                })}
              </tbody>
