...
import {
  ...
  Tag, AlertTriangle, ShieldCheck, CheckCircle2,
} from 'lucide-react';
...
export function TemplateEditor({ template, onClose, onCancel, isCopy = false, onSaveCopy, onSaveEdit, existingTemplateNames = [], initialName }: { template: EditableTemplate; onClose: () => void; onCancel?: () => void; isCopy?: boolean; onSaveCopy?: (copy: EditableTemplate) => void; onSaveEdit?: (updated: EditableTemplate) => void; existingTemplateNames?: string[]; initialName?: string }) {
  ...
  // Picking a type pre-fills the matching standard's sections. Don't clobber
  // existing work: on a fresh (empty) outline, fill the set; otherwise leave it
  // to the "add missing" affordance in the coverage panel.
  const onTypeChange = (next: ReportTypeName) => {
    setReportType(next);
    const std = typeSectionsFor(next);
    if (sections.length === 0 && std.length > 0) {
      setSections(std.map(s => ({ name: s.name, icon: s.icon })));
    }
  };
  const addSections = (list: { name: string; icon: string }[]) => {
    if (!list.length) return;
    setSections(prev => [...prev, ...list.map(s => ({ name: s.name, icon: s.icon }))]);
  };
      ...
      >
        ...
        <div className="flex-1 min-h-0 flex">
          ...
          <div>
            ...
            <select
              value={reportType}
              onChange={e => onTypeChange(e.target.value as ReportTypeName)}
              className="w-full px-3 py-2 rounded-[8px] border border-canvas-border text-[0.8125rem] bg-white cursor-pointer focus:outline-none focus:border-brand-600/40 focus:ring-2 focus:ring-brand-600/10"
            >
              {REPORT_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
            {/* Smart defaults (pre-fill + coverage) are a starting aid for new
                templates only. An existing/uploaded template's own sections are
                authoritative (PRD §4.6 caveat) — type is just its label there. */}
            <p className="text-[0.6875rem] text-ink-400 mt-1">
              {(() => {
                const hasStandard = typeSectionsFor(reportType).length > 0;
                if (!isNew) {
                  return hasStandard
                    ? `Suggests standard ${reportType} sections you can optionally add.`
                    : 'Sets the report type label for this template.';
                }
                if (reportType === 'Other') return 'Custom type — limited format checking.';
                return hasStandard
                  ? `Pre-fills the standard ${reportType} sections to start from.`
                  : `No standard ${reportType} template yet. Build sections manually or upload one.`;
              })()}
            </p>
          </div>
          <div>
            ...
            <div className="px-6 py-7">
              <div className="rounded-[14px] overflow-hidden border border-canvas-border bg-white shadow-[0_8px_28px_rgba(15,8,30,0.10)]">
                ...
                <div ref={sectionsRef} tabIndex={-1} className="px-8 py-6">
                  ...
                  {/* §4.6 section guidance for the chosen type. Two modes:
                      • New-from-scratch (isNew): full coverage — required (🔒 red /
                        ⚠️ amber) and recommended (muted) gaps; pairs with the
                        skippable save block.
                      • Existing / uploaded template: the user's sections are the
                        real format, so we never flag "required" there — only offer
                        the recommended sections as optional suggestions. */}
                  {coverage.spec.length > 0 && (
                    <div className="mb-4 rounded-[10px] border border-canvas-border bg-canvas/40 px-4 py-3">
                      <div className="flex items-center justify-between gap-3 mb-2">
                        <div className="flex items-center gap-2 text-[0.75rem] min-w-0">
                          ...
                          <span className="font-semibold text-ink-800 shrink-0">{isNew ? `${reportType} coverage` : `Recommended for ${reportType}`}</span>
                          <span className="text-ink-500 truncate">
                            {isNew
                              ? `${coverage.requiredPresent} of ${coverage.requiredTotal} required · ${coverage.recommendedPresent} of ${coverage.recommendedTotal} recommended`
                              : `${coverage.recommendedPresent} of ${coverage.recommendedTotal} recommended sections included`}
                          </span>
                        </div>
                        {(isNew ? coverage.allMissing : coverage.missingRecommended).length > 0 && (
                          <button
                            onClick={() => addSections(isNew ? coverage.allMissing : coverage.missingRecommended)}
                            className="inline-flex items-center gap-1 text-[0.6875rem] font-semibold text-brand-600 hover:text-brand-700 cursor-pointer shrink-0"
                          >
                            <Plus size={12} /> Add {(isNew ? coverage.allMissing : coverage.missingRecommended).length} {isNew ? 'missing' : 'suggested'}
                          </button>
                        )}
                      </div>
                      {isNew ? (
                        coverage.allMissing.length > 0 ? (
                          <div className="flex flex-wrap gap-1.5">
                            {coverage.missingRequired.map(s => (
                              <span key={s.name} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full border border-high/40 bg-high-50 text-[0.625rem] font-medium text-high-700"><AlertTriangle size={9} /> {s.name}</span>
                            ))}
                            {coverage.missingRecommended.map(s => (
                              <span key={s.name} className="inline-flex items-center px-2 py-0.5 rounded-full border border-canvas-border bg-white text-[0.625rem] font-medium text-ink-500">{s.name}</span>
                            ))}
                          </div>
                        ) : (
                          <div className="flex items-center gap-1.5 text-[0.6875rem] text-compliant-700"><CheckCircle2 size={11} /> All standard {reportType} sections present.</div>
                        )
                      ) : (
                        coverage.missingRecommended.length > 0 ? (
                          ...
                          <div className="flex items-center gap-1.5 text-[0.6875rem] text-compliant-700"><CheckCircle2 size={11} /> All recommended sections included.</div>
                        )
                      )}
                    </div>
                  )}
