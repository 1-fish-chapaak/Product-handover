...
export function PlanSection({ workflow }: { workflow: WorkflowDraft | null }) {
  ...
  return (
    <div className="rounded-lg border border-canvas-border bg-canvas-elevated p-4">
      <div className="text-[0.75rem] font-bold uppercase tracking-wider text-ink-400 mb-3">
        ...
        {workflow.steps.map((s, idx) => (
          <li key={s.id} className="flex items-start gap-3">
            <div className="flex flex-col items-center shrink-0">
              <div className="w-6 h-6 rounded-full bg-brand-50 text-brand-700 flex items-center justify-center text-[0.75rem] font-bold">
                {idx + 1}
              </div>
            ...
            <div className="min-w-0 pb-1">
              ...
              <div className="text-[0.75rem] text-ink-400 leading-relaxed">{s.description}</div>
            </div>
          </li>
        ))}
      </ol>
      <div className="mt-3 rounded-lg border border-compliant/40 bg-compliant-50 p-2.5 flex items-start gap-2">
        ...
        <div className="min-w-0">
          <div className="text-[0.75rem] font-bold uppercase tracking-wider text-compliant-700">
    ...
    <div className="rounded-lg border border-canvas-border bg-canvas-elevated p-4">
      ...
      <div className="mt-3 mb-3">
        <div className="flex items-center justify-between mb-1">
          <span className="text-[0.75rem] font-bold uppercase tracking-wider text-ink-400">
            Control Coverage
          </span>
      ...
      <div className="text-[0.75rem] font-bold uppercase tracking-wider text-ink-400 mb-2">
        Mapped Risks
      </div>
        ...
        {MAPPED_RISKS.map((r) => (
          ...
          >
            <div className="flex items-center gap-2 mb-1">
              ...
              <span className="text-[0.75rem] font-semibold text-ink-800">{r.id}</span>
              <span
            ...
            <div className="text-[0.75rem] text-ink-700 leading-snug mb-1">{r.title}</div>
...
function InputConfigSection({
  ...
  return (
    <div className="space-y-3">
      {parameters && onParametersChange && (
        <div className="rounded-xl border border-canvas-border bg-canvas-elevated">
          <div className="flex items-center gap-2 px-3 py-2.5 border-b border-canvas-border">
            ...
            <span className="text-[0.75rem] font-semibold text-brand-700 bg-brand-50 border border-brand-100 rounded-full px-1.5 py-0.5">
              LIVE
            </span>
          </div>
          <div className="p-3 space-y-3">
            <div>
              <label className="text-[0.75rem] font-semibold text-ink-600 flex items-center gap-1.5 mb-1">
              ...
              <div className="relative">
                <input
                  ...
                  className="w-full rounded-lg border border-canvas-border bg-canvas-elevated pl-2.5 pr-7 py-1.5 text-[0.75rem] font-mono text-ink-800 focus:outline-none focus:border-brand-400 transition-colors"
                />
                <span className="absolute right-2.5 top-1/2 -translate-y-1/2 text-[0.75rem] text-ink-400">%</span>
              </div>
            </div>
            <div>
              <label className="text-[0.75rem] font-semibold text-ink-600 flex items-center gap-1.5 mb-1">
              ...
              <div className="grid grid-cols-2 gap-1.5">
                <DatePicker
                  ...
                  className="rounded-lg border border-canvas-border bg-canvas-elevated px-2 py-1.5 text-[0.75rem] font-mono text-ink-800 focus:outline-none focus:border-brand-400 transition-colors"
                />
                <DatePicker
          ...
          {addingNote ? (
            <div className="rounded-lg border border-brand-200 bg-brand-50/20 p-2.5 space-y-1.5">
              <input
                ...
                autoFocus
                className="w-full rounded-md border border-canvas-border bg-canvas-elevated px-2 py-1.5 text-[0.75rem] font-semibold text-ink-800 placeholder:text-ink-400 focus:outline-none focus:border-brand-400 transition-colors"
              />
              <input
                type="text"
                value={newDesc}
                ...
                className="w-full rounded-md border border-canvas-border bg-canvas-elevated px-2 py-1.5 text-[0.75rem] text-ink-600 placeholder:text-ink-400 focus:outline-none focus:border-brand-400 transition-colors"
              />
              <div className="flex items-center gap-1.5 pt-0.5">
                <button
                  ...
                  className={`inline-flex items-center gap-1 rounded-md text-[0.75rem] font-semibold px-2 py-1 transition-colors ${
                    newName.trim()
                ...
                <button
                  ...
                  className="inline-flex items-center gap-1 rounded-md text-[0.75rem] font-semibold px-2 py-1 text-ink-500 hover:bg-canvas transition-colors cursor-pointer"
                >
                  <X size={10} />
                  Cancel
                </button>
      ...
      {workflow && (
        <div className="text-center text-[0.75rem] uppercase tracking-wider text-ink-400 font-semibold">
          For {workflow.name}
        </div>
      )}
    </div>
...
function OutputConfigSection({ workflow }: { workflow: WorkflowDraft | null }) {
  ...
  return (
    <div className="space-y-3">
      {/* Dashboard KPIs */}
      <div className="rounded-lg border border-canvas-border bg-canvas-elevated p-4">
        <div className="text-[0.8125rem] font-semibold text-ink-800 mb-3">Dashboard KPIs</div>
          ...
          {KPI_ITEMS.map(({ key, label, badge }) => {
            ...
            return (
              <label key={key} className="flex items-center gap-2.5 cursor-pointer group">
                ...
                <span
                  className={`text-[0.8125rem] flex-1 ${checked ? 'text-ink-700 font-semibold' : 'text-ink-400'}`}
                >
                  {label}
                </span>
                {badge && (
                  <span className="text-[0.75rem] font-bold px-1.5 py-0.5 rounded bg-mitigated-50 text-mitigated-700 border border-mitigated/30">
                    {badge}
                  </span>
                )}
              </label>
      ...
      {/* Output Layout */}
      <div className="rounded-lg border border-canvas-border bg-canvas-elevated p-4">
        <div className="text-[0.8125rem] font-semibold text-ink-800 mb-3">Output Layout</div>
      ...
      <div className="rounded-lg border border-canvas-border bg-canvas-elevated p-4">
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-[0.8125rem] font-semibold text-ink-800">Delivery & Routing</span>
          <span className="ml-auto text-[0.75rem] font-bold px-1.5 py-0.5 rounded bg-brand-50 text-brand-600 uppercase tracking-wide">
            New
          </span>
        </div>
        ...
        <p className="text-[0.75rem] font-bold text-ink-400 uppercase tracking-wider mb-2">
          ...
          {(Object.keys(CHANNEL_META) as ChannelKey[]).map((key) => {
            ...
            return (
              <button
                ...
                className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-full text-[0.75rem] font-semibold border transition-all cursor-pointer ${
                  active
                  ...
                  <div className="px-3 pb-3 space-y-2.5 border-t border-canvas-border pt-2.5">
                    <div>
                      ...
                      <input
                        ...
                        className="w-full mt-1 px-2.5 py-1.5 text-[0.75rem] border border-canvas-border rounded-md bg-canvas-elevated text-ink-700 placeholder:text-ink-300 focus:outline-none focus:border-brand-400 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="text-[0.75rem] font-bold text-ink-400 uppercase tracking-wider">
                      ...
                      <div className="flex gap-1.5 mt-1 flex-wrap">
                        {(
                          ...
                          <button
                            ...
                            className={`px-2.5 py-1 text-[0.75rem] rounded-md border transition-all cursor-pointer ${
