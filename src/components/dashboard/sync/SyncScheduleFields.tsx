...
import { CalendarClock, Clock, Calendar, CalendarDays, CalendarRange, CalendarCheck, CalendarFold, Repeat, Bell, Minus, Plus, Zap, Globe } from 'lucide-react';
import {
  type SyncSchedule, type SyncFrequency, type CustomUnit,
  DAY_INITIAL, summarizeSchedule, nextRun, fmtNextRun,
} from './syncSchedule';
...
/**
 * The cadence editor's fields — master switch, frequency grid, the detail
 * control each frequency needs, the failure notification, and the resulting
 * next occurrence. Layout only: the draft and the save live with whoever mounts
 * it (the Automatic-data-sync modal, or the Update-dashboard dialog's inline
 * "Bulk schedule runs" view).
 */
export default function SyncScheduleFields({ draft, onPatch, lastSyncedLabel }: {
  draft: SyncSchedule;
  onPatch: (patch: Partial<SyncSchedule>) => void;
