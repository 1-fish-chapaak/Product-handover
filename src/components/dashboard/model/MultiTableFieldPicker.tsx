...
export default function MultiTableFieldPicker({
  ...
  return (
    <div className="grid grid-cols-[1fr_1fr] gap-3 h-full min-h-0">
      {/* Table / field tree */}
      <div className="border border-border-light rounded-lg overflow-y-auto">
        {tables.map(t => {
          ...
          return (
            <div key={t.id} className="border-b border-border-light/60 last:border-b-0">
              <button onClick={() => setExpanded(prev => { const n = new Set(prev); if (n.has(t.id)) n.delete(t.id); else n.add(t.id); return n; })} className="w-full flex items-center gap-2 px-3 py-2 hover:bg-paper-50 cursor-pointer">
                ...
                <span className="text-[0.78125rem] font-semibold text-text flex-1 text-left truncate">{t.name}</span>
                {selectedTables.length > 0 && conn && !selectedTables.includes(t.id) && (
                  <span className="inline-flex items-center gap-1 h-5 px-1.5 text-[0.59375rem] font-semibold bg-compliant-50 text-compliant-700 rounded-full"><Link2 size={9} /> connected</span>
                )}
              </button>
              {open && (
                  ...
                  {t.columns.map(c => {
                    ...
                    return (
                      <button key={c.name} onClick={() => toggleField(t.id, c.name, c.role)} className={`w-full flex items-center gap-2 pl-8 pr-3 py-1.5 text-left cursor-pointer ${on ? 'bg-primary-xlight/50' : 'hover:bg-paper-50'}`}>
                        ...
                        <span className="text-[0.75rem] text-text flex-1 truncate">{c.label}</span>
      ...
      {/* Selected fields */}
      <div className="border border-border-light rounded-lg p-3 overflow-y-auto flex flex-col gap-3">
        {notConnected && (
          <div className="rounded-md bg-mitigated-50 border border-mitigated/30 px-2.5 py-2">
            <p className="text-[0.71875rem] text-mitigated-700 leading-snug mb-1.5">These tables aren't connected, so they can't be combined yet.</p>
            <button onClick={onConnectTables} className="inline-flex items-center gap-1 h-7 px-2.5 text-[0.6875rem] font-semibold text-primary bg-white border border-primary/20 rounded-md hover:bg-primary-xlight cursor-pointer"><Link2 size={11} /> Connect tables</button>
          </div>
        )}

        <div>
          <div className="text-[0.65625rem] font-semibold uppercase tracking-wide text-text-muted mb-1.5">Group by (dimensions)</div>
          {dims.length === 0 ? <p className="text-[0.6875rem] text-text-muted/70">Add a text/category field — e.g. Vendor Name, Month.</p> : (
            <div className="flex flex-wrap gap-1.5">
              {dims.map(f => {
                const idx = selected.indexOf(f);
                return (
                  <span key={`${f.table}.${f.column}`} className="inline-flex items-center gap-1.5 h-7 pl-2 pr-1 bg-paper-50 border border-border-light rounded-md text-[0.71875rem] text-text">
                    <span className="text-text-muted text-[0.625rem]">{tableName(f.table)} ·</span> {labelFor(f.table, f.column)}
        ...
        <div>
          <div className="text-[0.65625rem] font-semibold uppercase tracking-wide text-text-muted mb-1.5">Values (measures)</div>
          {measures.length === 0 ? <p className="text-[0.6875rem] text-text-muted/70">Add a numeric field — e.g. Invoice Amount, Duplicate Count.</p> : (
            <div className="space-y-1.5">
              {measures.map(f => {
                const idx = selected.indexOf(f);
                return (
                  <div key={`${f.table}.${f.column}`} className="flex items-center gap-1.5">
                    <select value={f.agg ?? 'sum'} onChange={e => setAgg(idx, e.target.value as AggFn)} className="h-7 px-1.5 bg-white border border-border-light rounded-md text-[0.6875rem] text-text cursor-pointer focus:outline-none focus:border-primary/40">
                      {AGGS.map(a => <option key={a.value} value={a.value}>{a.label}</option>)}
                    </select>
                    <span className="flex-1 inline-flex items-center gap-1 h-7 px-2 bg-paper-50 border border-border-light rounded-md text-[0.71875rem] text-text min-w-0">
                      <span className="text-text-muted text-[0.625rem] shrink-0">{tableName(f.table)} ·</span> <span className="truncate">{labelFor(f.table, f.column)}</span>
                    </span>
