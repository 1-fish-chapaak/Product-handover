...
function Select({ value, onChange, placeholder, options }: { value: string; onChange: (v: string) => void; placeholder: string; options: { value: string; label: string }[] }) {
  return (
    <select value={value} onChange={e => onChange(e.target.value)} className="w-full h-9 px-2.5 bg-white border border-border-light rounded-md text-[0.78125rem] text-text focus:outline-none focus:border-primary/40 cursor-pointer">
...
export default function RelationshipEditor({
  ...
  return (
    <div className="p-5">
      <h4 className="text-[0.875rem] font-semibold text-text mb-1">{initial ? 'Edit connection' : 'New connection'}</h4>
      <p className="text-[0.75rem] text-text-muted mb-4">Pick two tables and the column(s) that match between them. Rows are combined where those columns are equal.</p>
      ...
      {lt && rt && (
        <div className="space-y-2 mb-3">
          <div className="text-[0.6875rem] font-semibold uppercase tracking-wide text-text-muted">Matching columns</div>
          {pairs.map((p, i) => (
          ...
          <button onClick={() => setPairs(prev => [...prev, { left: '', right: '' }])} className="inline-flex items-center gap-1 text-[0.75rem] font-medium text-primary hover:text-primary-hover cursor-pointer">
      ...
      <label className={`flex items-center gap-2 mb-1 ${otherActive ? 'opacity-60' : 'cursor-pointer'}`}>
        ...
        <span className="text-[0.78125rem] text-text">Use this connection when combining these tables (active)</span>
      </label>
      {otherActive && (
        <p className="text-[0.71875rem] text-mitigated-700 bg-mitigated-50 border border-mitigated/30 rounded-md px-2.5 py-1.5 inline-flex items-start gap-1.5 mb-1">
      ...
      <div className="flex items-center justify-end gap-2 mt-4 pt-3 border-t border-border-light">
        <button onClick={onCancel} className="h-9 px-4 text-[0.78125rem] font-medium text-text bg-white border border-border-light rounded-md hover:bg-paper-50 cursor-pointer">Cancel</button>
        <button
          onClick={() => valid && onSave({ id: initial?.id ?? `rel-${Date.now()}`, leftTable, rightTable, columnPairs: pairs, active: effectiveActive })}
          disabled={!valid}
          className="h-9 px-5 text-[0.78125rem] font-semibold text-white bg-primary hover:bg-primary-hover rounded-md cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
        >
