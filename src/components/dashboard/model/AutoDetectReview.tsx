  ...
  return (
    <div className="p-5">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h4 className="text-[0.875rem] font-semibold text-text">Suggested connections</h4>
      </div>
      ...
      <div className="space-y-2 max-h-[320px] overflow-y-auto">
        {candidates.map(c => {
          ...
          return (
            <button key={c.id} onClick={() => toggle(c.id)} className={`w-full text-left flex items-center gap-3 px-3 py-2.5 rounded-md border transition-colors cursor-pointer ${on ? 'border-primary/40 bg-primary-xlight/40' : 'border-border-light hover:border-primary/20'}`}>
              <span className={`w-4 h-4 rounded-sm border flex items-center justify-center shrink-0 ${on ? 'bg-primary border-primary' : 'border-border'}`}>{on && <Check size={11} className="text-white" />}</span>
              <div className="flex-1 min-w-0">
                ...
                <div className="text-[0.6875rem] text-text-muted mt-0.5">{c.reason}</div>
              </div>
              {c.willBeInactive && (
                <span className="shrink-0 inline-flex items-center h-5 px-2 text-[0.625rem] font-semibold bg-mitigated-50 text-mitigated-700 rounded-full">Will be Inactive</span>
              )}
            </button>
          );
        })}
      ...
      <div className="flex items-center justify-between gap-2 mt-4 pt-3 border-t border-border-light">
        <span className="text-[0.71875rem] text-text-muted">{picked.size} selected</span>
