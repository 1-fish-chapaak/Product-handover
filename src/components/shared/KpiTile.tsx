...
export function KpiTile({ label, value, index = 0, onClick, editing, footer, className = '' }: KpiTileProps) {
  ...
  return (
    <motion.div
      ...
      className={`glass-card rounded-xl px-5 py-4 transition-[border-color,box-shadow] duration-300 ${onClick ? 'cursor-pointer hover:border-brand-200 hover:shadow-[0_12px_28px_-14px_rgba(15,8,30,0.22)]' : 'cursor-default'} ${className}`}
    >
      {editing ?? (
        <>
