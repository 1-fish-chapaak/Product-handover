...
export default function SetupWizard() {
  ...
  return (
    <div className="max-w-[820px] mx-auto space-y-5">
      <button onClick={back} className="inline-flex items-center gap-1.5 text-[0.78125rem] font-semibold text-ink-500 hover:text-brand-700 cursor-pointer transition-colors"><ArrowLeft size={14} /> Control register</button>
      <div>
        <h1 className="text-[1.375rem] font-bold text-ink-900 tracking-tight" style={{ fontFamily: "'Source Serif 4', Georgia, serif" }}>New ICFR engagement</h1>
        <p className="text-[0.8125rem] text-ink-500 mt-0.5">Scope it once — materiality, accounts, RACM, owners — then test.</p>
      </div>
        ...
        {STEPS.map((s, i) => (
          <div key={s} className="flex items-center flex-1 last:flex-none">
            <div className="flex items-center gap-1.5 shrink-0">
              <span className={cn('inline-flex items-center justify-center w-6 h-6 rounded-full text-[0.6875rem] font-bold', i < step ? 'bg-brand-600 text-white' : i === step ? 'bg-brand-100 text-brand-700 ring-2 ring-brand-200' : 'bg-paper-100 text-ink-400')}>{i < step ? <Check size={13} /> : i + 1}</span>
              <span className={cn('text-[0.6875rem] font-semibold whitespace-nowrap', i === step ? 'text-ink-800' : 'text-ink-400')}>{s}</span>
            </div>
      ...
      <div className="rounded-lg border border-canvas-border bg-canvas-elevated p-5 min-h-[320px]">
          ...
          <motion.div key={step} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} transition={{ duration: 0.16 }}>
            ...
            {step === 1 && (
              <Section icon={<Target size={15} className="text-brand-600" />} title="Materiality" hint="Drives which accounts and controls fall in scope.">
                ...
                <p className="text-[0.75rem] text-ink-500 mt-3">Overall {formatINR(materiality)} · performance {formatINR(pm)}</p>
              </Section>
            )}
            {step === 2 && (
                ...
                <div className="space-y-2">
                  {TEMPLATE_ACCOUNTS.map(a => (
                    <label key={a.id} className="flex items-center gap-3 rounded-lg border border-canvas-border px-3 py-2.5 cursor-pointer hover:bg-paper-50">
                      ...
                      <span className="text-[0.8125rem] font-medium text-ink-800 flex-1">{a.name}</span>
                      <span className="text-[0.75rem] tabular-nums text-ink-500">{formatINR(a.balance)}</span>
                      <span className="text-[0.6875rem] text-ink-400">{a.assertions.join(' · ')}</span>
                    </label>
                  ))}
                </div>
              </Section>
            ...
            {step === 3 && (
              <Section icon={<FileText size={15} className="text-brand-600" />} title="RACM" hint="Pick a process — its key controls import as a template.">
                <div className="flex items-center gap-2 mb-3">
                  <span className="text-[0.75rem] text-ink-500">Process</span>
                  {PROCESSES.map(p => <button key={p} onClick={() => setProcess(p)} className={cn('h-8 px-3 rounded-md border text-[0.78125rem] font-semibold cursor-pointer', process === p ? 'bg-brand-50 border-brand-200 text-brand-700' : 'border-canvas-border text-ink-600 hover:bg-paper-50')}>{p}</button>)}
                  <span className="text-[0.75rem] text-ink-400 ml-2">{controls.length} key controls</span>
                </div>
                {controls.length === 0 ? <p className="text-[0.78125rem] text-ink-400">No template for {process} yet — pick P2P.</p> : (
                  <div className="space-y-1.5 max-h-[260px] overflow-y-auto">
                    {controls.map(c => <div key={c.id} className="rounded-lg border border-canvas-border px-3 py-2 text-[0.78125rem]"><span className="font-mono text-[0.6875rem] text-brand-700 mr-2">{c.id}</span>{c.description} <span className="text-ink-400">· {c.nature}</span></div>)}
                  </div>
                )}
              </Section>
            )}
                ...
                <div className="space-y-2 max-h-[300px] overflow-y-auto">
                  {controls.map(c => (
                    <div key={c.id} className="flex items-center gap-3 rounded-lg border border-canvas-border px-3 py-2">
                      <span className="font-mono text-[0.6875rem] text-brand-700 w-[78px] shrink-0">{c.id}</span>
                      <span className="text-[0.78125rem] text-ink-700 flex-1 min-w-0 truncate">{c.description}</span>
                      <select value={owners[c.id] ?? c.owner} onChange={e => setOwners(p => ({ ...p, [c.id]: e.target.value }))} className="h-8 px-2 rounded-md border border-canvas-border text-[0.75rem] bg-white cursor-pointer shrink-0">
            ...
            {step === 5 && (
              <Section icon={<ShieldCheck size={15} className="text-brand-600" />} title="Review & create">
                <dl className="grid grid-cols-2 gap-3 text-[0.78125rem]">
      ...
      <div className="flex items-center justify-between">
        <button onClick={prev} className="inline-flex items-center gap-1.5 h-9 px-3 rounded-lg text-[0.8125rem] font-semibold text-ink-600 hover:bg-paper-50 cursor-pointer transition-colors"><ArrowLeft size={15} /> {step === 0 ? 'Cancel' : 'Back'}</button>
        {step < STEPS.length - 1 ? (
          <button onClick={next} disabled={!canNext} className="inline-flex items-center gap-1.5 h-9 px-4 rounded-lg bg-brand-600 text-white text-[0.8125rem] font-semibold hover:bg-brand-500 cursor-pointer disabled:bg-brand-100 disabled:text-brand-300 disabled:cursor-not-allowed transition-colors">Continue <ArrowRight size={15} /></button>
        ) : (
          <button onClick={create} className="inline-flex items-center gap-1.5 h-9 px-5 rounded-lg bg-brand-600 text-white text-[0.8125rem] font-semibold hover:bg-brand-500 cursor-pointer transition-colors"><Check size={15} /> Create engagement</button>
        )}
      </div>
    </div>
  );
...
const inputCls = 'w-full h-9 px-3 rounded-lg border border-canvas-border text-[0.8125rem] text-ink-800 bg-white focus:outline-none focus:border-brand-300 focus:ring-2 focus:ring-brand-50';

function Section({ icon, title, hint, children }: { icon: React.ReactNode; title: string; hint?: string; children: React.ReactNode }) {
  return (
    <div>
      <h3 className="inline-flex items-center gap-2 text-[0.9375rem] font-semibold text-ink-900">{icon}{title}</h3>
      {hint && <p className="text-[0.78125rem] text-ink-500 mt-0.5 mb-4">{hint}</p>}
...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return <div><div className="text-[0.6875rem] font-semibold text-ink-500 mb-1">{label}</div>{children}</div>;
}
function Rev({ k, v }: { k: string; v: string }) {
  return <div className="rounded-lg border border-canvas-border px-3 py-2"><div className="text-[0.65625rem] uppercase tracking-wide text-ink-400 font-semibold">{k}</div><div className="text-ink-800 font-medium mt-0.5">{v}</div></div>;
}
