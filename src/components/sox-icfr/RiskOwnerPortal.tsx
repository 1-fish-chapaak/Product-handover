  ...
  return (
    <div className="max-w-[760px] mx-auto space-y-5">
      <div>
        ...
        <p className="text-[0.8125rem] text-ink-500 mt-0.5">{open.length} open · what the audit team needs from you, and by when.</p>
      </div>
      ...
      {open.length === 0 ? (
        ...
        <div className="space-y-3">
          {open.map(t => {
            ...
            return (
              <div key={t.id} className="rounded-lg border border-canvas-border bg-canvas-elevated p-4 flex items-start gap-3.5">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    ...
                    <span className={cn('ml-auto text-[0.71875rem] font-medium inline-flex items-center gap-1', t.overdue ? 'text-risk-700' : 'text-ink-500')}><Clock size={12} />{t.dueLabel}</span>
                  </div>
                  ...
                  <div className="flex items-center gap-2 mt-3">
                    <button onClick={() => act(t)} className="inline-flex items-center gap-1.5 h-9 px-4 rounded-lg bg-brand-600 text-white text-[0.8125rem] font-semibold hover:bg-brand-500 cursor-pointer transition-colors"><m.Icon size={14} /> {m.action}</button>
                    <button className="inline-flex items-center gap-1.5 h-9 px-3 rounded-lg border border-canvas-border text-[0.8125rem] font-semibold text-ink-600 hover:border-brand-300 cursor-pointer transition-colors"><MessageSquare size={14} /> Comment</button>
                  </div>
                </div>
              </div>
            );
