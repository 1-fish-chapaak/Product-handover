...
| `NotificationBell.tsx` · `NotificationPopover.tsx` · `NotificationHeader.tsx` · `NotificationList.tsx` · `NotificationItem.tsx` · `NotificationIcon.tsx` · `NotificationTimestamp.tsx` · `NotificationEmptyState.tsx` | The notification centre. The bell (sidebar) announces "Notifications, N unread" and tilts once when the count rises. The popover anchors beside the bell (bottom sheet under 640px): **Notifications · N unread**, **All | Unread**, rows, **Mark all as read**. A row is *what happened · why it matters · when* and the whole row is the one control — click marks it read and deep-links via `link { view, ref }`. No inline actions, no metadata pills, no email tab: approvals live in Exceptions Management, emails in Preferences → Preview. Opening the popover marks nothing read. Tokens: `--notification-*` in `index.css`, all derived from the palette; `prefers-reduced-motion` zeroes the durations. |
