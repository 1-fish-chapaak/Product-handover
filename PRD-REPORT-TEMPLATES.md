...
| SOX Compliance Report | ✅ Shipped — query-wizard generate | 7 sections (incl. Testing Methodology); queries slot under Control Testing Results; Deficiency/Remediation/Appendix follow |
...
1. **Step 1 — Pick queries.** One picker, three tabs: *Reports* / *Recent Chats* (IRA chat queries) / *Workflows*. The Reports tab groups queries **under the report they live in** (a report holds many queries); the group header checkbox takes or drops the whole report, with indeterminate state for partial picks. Recent Chats is the same list every picker shows — sourced from the canonical `QUERY_SESSIONS` history (`queryHistory.ts`, shared with the Add Query modal), grouped Today / Yesterday / Last 7 days, with the Add Query modal's exact row styling; only prompts that map to rich query content are offered. The Workflows tab lists **only workflows with a finished run that produced query results** (WF id, type, last run, run count — same data as the workflow library); picking one adds the result query from its latest finished run. **Selecting 2+ workflows = a bulk audit**: the wizard flags it and appends the cross-workflow rollup query at Continue (skipped if the same query is already selected from another tab). User assembles any mix; selected queries collect into a tray with a running count. Minimum one query to proceed — there is no skip/empty path (removed 12 Jun 2026; "no empty reports" now holds with zero exceptions).
...
- **Customize** on any card → existing TemplateEditor (brand, theme, header/footer, arrangement). Editing a standard template edits it in place; **Save as copy** remains available in the same editor.
