...
test('member drawer reconciles with the table row and links to Admin', async ({ page }) => {
  ...
  // Stats-only page: the drawer has no action buttons
  await row.click();
  await page.waitForTimeout(500);
  await expect(drawer).toBeVisible();
  await expect(drawer.getByRole('button', { name: 'Manage in Admin', exact: true })).not.toBeVisible();
});
...
test('depth: highlights, rhythm, module drawer, segments, team drawer', async ({ page }) => {
  ...
  for (const s of ['Engagements', 'Ask IRA & AI tools', 'Reports', 'Workflows', 'Risk & Controls', 'Knowledge Hub', 'Dashboards']) {
    await expect(page.locator('h3', { hasText: s }).first()).toBeVisible();
  }

  // Tile → modal → full detail, Esc closes
  await page.locator('h3', { hasText: 'Ask IRA & AI tools' }).click();
  await page.waitForTimeout(600);
  const aiModal = page.getByRole('dialog', { name: 'Ask IRA & AI tools' });
  await expect(aiModal).toBeVisible();
  await expect(aiModal.getByText('Who uses AI most')).toBeVisible();
  await page.keyboard.press('Escape');
  await page.waitForTimeout(400);
