...
//  4. "Build a workflow" mode control — clickable on the start composer (two states),
//     read-only (role=status, not a button) inside an open chat.
...
test('Fix 4 — start composer: Build-a-workflow toggle, Q&A vs Workflow', async ({ page }) => {
  await page.goto('/?view=chat');
  await enterWorkspace(page);

  const modeBtn = page.getByRole('button', { name: 'Build a workflow' });
  await expect(modeBtn).toBeVisible();
  await expect(modeBtn).toHaveAttribute('aria-pressed', 'false'); // Q&A by default
  await page.waitForTimeout(2200); // let the placeholder type in
  await page.screenshot({ path: 'test-results/chat-composer-qna.png' });

  await modeBtn.click();
  await expect(modeBtn).toHaveAttribute('aria-pressed', 'true'); // workflow on
...
test('Fix 4 — in-chat composer shows a read-only mode indicator (after clarification)', async ({ page }) => {
  ...
  // It must NOT be a clickable button anymore (read-only in-thread).
  await expect(page.getByRole('button', { name: 'Build a workflow' })).toHaveCount(0);
