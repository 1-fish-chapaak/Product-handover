...
Every status chip renders through one canonical base — **`Pill`**: `inline-flex items-center px-2.5 h-6 rounded-full text-[0.75rem] leading-[16px] whitespace-nowrap tabular-nums`, full radius, title-case, **no icon**. **`variant="bordered"` is the default and the common status used everywhere** — semibold + a soft tone border (`border-{tone}/30`) over the tone pair. 7 tones: `risk` `bg-risk-50 text-risk-700`, `high` `bg-high-50 text-high-700`, `mitigated` `bg-mitigated-50 text-mitigated-700`, `compliant` `bg-compliant-50 text-compliant-700`, `evidence` `bg-evidence-50 text-evidence-700`, `info` `bg-brand-50 text-brand-700`, `draft` `bg-draft-50 text-draft-700`. `variant="flat"` (no border, `font-medium`) is the quiet opt-out. Labels always spelled out (`Critical`, not `C`; the RACM key-control marker is the spelled-out **Key** pill — `FrameworkBadge` tone `high` — never a round single-letter `K`); never arranged as a RAG strip. **Raw-data cells are exempt:** a query-result data table (e.g. the chat `ResultsTable`) renders literal cell values — including a `Status` column like "Open / In review / Resolved" — as **plain text, not `StatusBadge` pills**, so the table stays column-agnostic and matches the CSV / HTML export.

Seven **domain badges** map a vocabulary → tone+label on that base — **use these, never hand-roll an inline status span:**
- **`StatusBadge`** — 22 lifecycle states (active→compliant, open→risk, in-progress→evidence, resolved/effective→compliant, invited→info, suspended/expired→high, locked/blocked→risk, pending→mitigated, draft/inactive/not-started/not-tested→draft, …), **plus the RACM lifecycle** — `Draft`→draft, `In Progress`→evidence, `Active`→compliant, `Locked`→risk, `Archived`→draft — and **RACM readiness** — `Ready`→compliant, `Mapping Incomplete` / `Workflow Missing` / `Configuration Pending`→mitigated. ⚠️ `audit/racmStateEngine.ts` currently hand-maps these to raw `blue` / `emerald` / `purple` / `amber` palette classes — route them through these semantic tones instead (open follow-up).
...
- **`ConfidenceBadge`** — RACM extraction provenance (how a row was sourced, *not* a severity): `Extracted`→compliant, `Inferred`→mitigated, `Recommended`→info, `Draft`→draft. Labels spelled out title-case; source enum is `EXTRACTED` / `INFERRED` / `RECOMMENDED` / `DRAFT`. Provenance deliberately avoids the `risk`/`high` severity tones so it never reads as a finding.
...
A tool-launcher landing for the AI specialist tools. **Flat editorial, no glow** — this follows the standard surface recipe, *not* a decorative exception: a full-bleed `canvas-elevated` header band (serif `font-display` H1 + `FloatingLines` ambient, the one sanctioned motion) over the warm `canvas` + tinted mesh, then a 3-up tile grid.

Tiles (`ToolCard`, local): flat `canvas-elevated` sheet, 1px `canvas-border` tinting to `brand-300` on hover, `rounded-lg`, whisper resting shadow + a single diffuse lift on hover (the one earned lift, §4); a **per-tool pastel icon chip** (soft Tailwind `-100` surface + matching `-600` icon — a decorative accent; tone-colored icon tiles have precedent in §7.4.3, though the stricter DS norm is the brand chip / semantic tones); Inter title + 2-line description; **uniform brand tag chips** (`bg-brand-50` / `text-brand-700`, `rounded-md`), all shown inline. **No glassmorphism, no gradient panel, no cursor spotlight, no glow** — an earlier frosted-glass port (and the legacy "GlowCard glow" idea) was reverted as off-system per §1 / §4 (No-Glow) / §6 (Don't use glassmorphism, Single Gradient Rule).

Seven tiles, each routing to its own job-engine view under `intelligence/concierge/`: **RACM Generator** (first), Document Forensics, Table Extractor, Insights & Anomaly, Image Analytics, Speech Auditor, Medical Report Reader. (A Workflow Builder tile is currently commented out of the grid — workflow building stays reachable in Ask IRA chat, §7.15.3.) Each tool view composes the shared `ConciergeFlow` (upload → mock job lifecycle → result + History). Status/severity inside the tools should route through `SeverityBadge` / `Pill` (§7.10.4), never hand-rolled spans.
